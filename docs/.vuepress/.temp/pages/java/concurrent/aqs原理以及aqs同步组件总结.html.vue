...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">//返回同步状态的当前值</span>
...
<blockquote>
<p>说明：下面这部分关于 <code v-pre>ReentrantLock</code> 源代码内容节选自：https://www.javadoop.com/post/AbstractQueuedSynchronizer-2 ，这是一篇很不错文章，推荐阅读。</p>
</blockquote>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token doc-comment comment">/** Synchronizer providing all implementation mechanics */</span>
...
<p>举个很简单的例子假如我们要去一个地方的步骤是：购票 <code v-pre>buyTicket()</code>-&gt;安检 <code v-pre>securityCheck()</code>-&gt;乘坐某某工具回家 <code v-pre>ride()</code> -&gt;到达目的地 <code v-pre>arrive()</code>。我们可能乘坐不同的交通工具回家比如飞机或者火车，所以除了<code v-pre>ride()</code>方法，其他方法的实现几乎相同。我们可以定义一个包含了这些方法的抽象类，然后用户根据自己的需要继承该抽象类然后修改 <code v-pre>ride()</code>方法。</p>
</blockquote>
...
<p>以 <code v-pre>ReentrantLock</code> 为例，state 初始化为 0，表示未锁定状态。A 线程 <code v-pre>lock()</code> 时，会调用 <code v-pre>tryAcquire()</code>独占该锁并将 state+1。此后，其他线程再 <code v-pre>tryAcquire()</code> 时就会失败，直到 A 线程 unlock()到 state=0（即释放锁）为止，其它线程才有机会获取该锁。当然，释放锁之前，A 线程自己是可以重复获取此锁的（state 会累加），这就是可重入的概念。但要注意，获取多少次就要释放多么次，这样才能保证 state 是能回到零态的。</p>
<p>再以 <code v-pre>CountDownLatch</code> 以例，任务分为 N 个子线程去执行，state 也初始化为 N（也可以不初始化为 N，不初始化为 N,state 减到 0 也会从 await()返回）。这 N 个子线程是并行执行的，每个子线程执行完后 <code v-pre>countDown()</code> 一次，state 会 CAS(Compare and Swap)减 1。等到 <code v-pre>state=0</code>，会 <code v-pre>unpark()</code> 主调用线程，然后主调用线程就会从 <code v-pre>await()</code> 函数返回，继续后余动作。</p>
<p>所以 <code v-pre>CountDownLatch</code> 可以做倒计数器，减到 0 后唤醒的线程可以对线程池进行处理，比如关闭线程池。</p>
...
<ul>
<li><a href="https://www.cnblogs.com/waterystone/p/4920797.html" target="_blank" rel="noopener noreferrer">Java 并发之 AQS 详解<ExternalLinkIcon/></a></li>
<li><a href="https://www.cnblogs.com/chengxiao/p/7141160.html" target="_blank" rel="noopener noreferrer">Java 并发包基石-AQS 详解<ExternalLinkIcon/></a></li>
</ul>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>threadnum:0is ready
threadnum:1is ready
threadnum:2is ready
threadnum:3is ready
threadnum:4is ready
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>    <span class="token comment">// 当线程数量或者请求数量达到 count 时 await 之后的方法才会被执行。上面的示例中 count 的值就为 5。</span>
