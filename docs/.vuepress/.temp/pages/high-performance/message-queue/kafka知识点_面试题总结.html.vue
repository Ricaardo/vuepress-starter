<template><div><h1 id="kafka知识点-面试题总结" tabindex="-1"><a class="header-anchor" href="#kafka知识点-面试题总结" aria-hidden="true">#</a> Kafka知识点&amp;面试题总结</h1>
<h3 id="kafka-是什么-主要应用场景有哪些" tabindex="-1"><a class="header-anchor" href="#kafka-是什么-主要应用场景有哪些" aria-hidden="true">#</a> Kafka 是什么？主要应用场景有哪些？</h3>
...
<h3 id="和其他消息队列相比-kafka的优势在哪里" tabindex="-1"><a class="header-anchor" href="#和其他消息队列相比-kafka的优势在哪里" aria-hidden="true">#</a> 和其他消息队列相比,Kafka的优势在哪里？</h3>
...
<h3 id="队列模型了解吗-kafka-的消息模型知道吗" tabindex="-1"><a class="header-anchor" href="#队列模型了解吗-kafka-的消息模型知道吗" aria-hidden="true">#</a> 队列模型了解吗？Kafka 的消息模型知道吗？</h3>
<blockquote>
<p>题外话：早期的 JMS 和 AMQP 属于消息服务领域权威组织所做的相关的标准，我在 <a href="https://github.com/Snailclimb/JavaGuide" target="_blank" rel="noopener noreferrer">JavaGuide<ExternalLinkIcon/></a>的 <a href="https://github.com/Snailclimb/JavaGuide#%E6%95%B0%E6%8D%AE%E9%80%9A%E4%BF%A1%E4%B8%AD%E9%97%B4%E4%BB%B6" target="_blank" rel="noopener noreferrer">《消息队列其实很简单》<ExternalLinkIcon/></a>这篇文章中介绍过。但是，这些标准的进化跟不上消息队列的演进速度，这些标准实际上已经属于废弃状态。所以，可能存在的情况是：不同的消息队列都有自己的一套消息模型。</p>
</blockquote>
<h4 id="队列模型-早期的消息模型" tabindex="-1"><a class="header-anchor" href="#队列模型-早期的消息模型" aria-hidden="true">#</a> 队列模型：早期的消息模型</h4>
...
<h3 id="什么是producer、consumer、broker、topic、partition" tabindex="-1"><a class="header-anchor" href="#什么是producer、consumer、broker、topic、partition" aria-hidden="true">#</a> 什么是Producer、Consumer、Broker、Topic、Partition？</h3>
...
<h3 id="kafka-的多副本机制了解吗-带来了什么好处" tabindex="-1"><a class="header-anchor" href="#kafka-的多副本机制了解吗-带来了什么好处" aria-hidden="true">#</a> Kafka 的多副本机制了解吗？带来了什么好处？</h3>
...
<h3 id="zookeeper-在-kafka-中的作用知道吗" tabindex="-1"><a class="header-anchor" href="#zookeeper-在-kafka-中的作用知道吗" aria-hidden="true">#</a> Zookeeper 在 Kafka 中的作用知道吗？</h3>
<blockquote>
...
<ol>
<li><strong>Broker 注册</strong> ：在 Zookeeper 上会有一个专门<strong>用来进行 Broker 服务器列表记录</strong>的节点。每个 Broker 在启动时，都会到 Zookeeper 上进行注册，即到 <code v-pre>/brokers/ids</code> 下创建属于自己的节点。每个 Broker 就会将自己的 IP 地址和端口等信息记录到该节点中去</li>
<li><strong>Topic 注册</strong> ： 在 Kafka 中，同一个<strong>Topic 的消息会被分成多个分区</strong>并将其分布在多个 Broker 上，<strong>这些分区信息及与 Broker 的对应关系</strong>也都是由 Zookeeper 在维护。比如我创建了一个名字为 my-topic 的主题并且它有两个分区，对应到 zookeeper 中会创建这些文件夹：<code v-pre>/brokers/topics/my-topic/Partitions/0</code>、<code v-pre>/brokers/topics/my-topic/Partitions/1</code></li>
...
<p>生产者(Producer) 调用<code v-pre>send</code>方法发送消息之后，消息可能因为网络问题并没有发送过去。</p>
<p>所以，我们不能默认在调用<code v-pre>send</code>方法发送消息之后消息发送成功了。为了确定消息是发送成功，我们要判断消息发送的结果。但是要注意的是  Kafka 生产者(Producer) 使用  <code v-pre>send</code> 方法发送消息实际上是异步的操作，我们可以通过 <code v-pre>get()</code>方法获取调用结果，但是这样也让它变为了同步操作，示例代码如下：</p>
<blockquote>
<p><strong>详细代码见我的这篇文章：<a href="https://mp.weixin.qq.com/s?__biz=Mzg2OTA0Njk0OA==&amp;mid=2247486269&amp;idx=2&amp;sn=ec00417ad641dd8c3d145d74cafa09ce&amp;chksm=cea244f6f9d5cde0c8eb233fcc4cf82e11acd06446719a7af55230649863a3ddd95f78d111de&amp;token=1633957262&amp;lang=zh_CN#rd" target="_blank" rel="noopener noreferrer">Kafka系列第三篇！10 分钟学会如何在 Spring Boot 程序中使用 Kafka 作为消息队列?<ExternalLinkIcon/></a></strong></p>
</blockquote>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但是一般不推荐这么做！可以采用为其添加回调函数的形式，示例代码如下：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果消息发送失败的话，我们检查失败的原因之后重新发送即可！</p>
<p><strong>另外这里推荐为 Producer 的<code v-pre>retries </code>（重试次数）设置一个比较合理的值，一般是 3 ，但是为了保证消息不丢失的话一般会设置比较大一点。设置完成之后，当出现网络问题之后能够自动重试消息发送，避免消息丢失。另外，建议还要设置重试间隔，因为间隔太小的话重试的效果就不明显了，网络波动一次你3次一下子就重试完了</strong></p>
...
<li>将 <strong><code v-pre>enable.auto.commit</code></strong> 参数设置为 false，关闭自动提交，开发者在代码中手动提交 offset。那么这里会有个问题：<strong>什么时候提交offset合适？</strong>
<ul>
