<template><div><h1 id="认证授权基础" tabindex="-1"><a class="header-anchor" href="#认证授权基础" aria-hidden="true">#</a> 认证授权基础</h1>
<h2 id="认证-authentication-和授权-authorization-的区别是什么" tabindex="-1"><a class="header-anchor" href="#认证-authentication-和授权-authorization-的区别是什么" aria-hidden="true">#</a> 认证 (Authentication) 和授权 (Authorization)的区别是什么？</h2>
...
<li>使用 <code v-pre>Cookie</code> 保存 <code v-pre>Session</code> 或者 <code v-pre>Token</code> ，向后端发送请求的时候带上 <code v-pre>Cookie</code>，这样后端就能取到 <code v-pre>Session</code> 或者 <code v-pre>Token</code> 了。这样就能记录用户当前的状态了，因为 HTTP 协议是无状态的。</li>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@GetMapping</span><span class="token punctuation">(</span><span class="token string">"/change-username"</span><span class="token punctuation">)</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@GetMapping</span><span class="token punctuation">(</span><span class="token string">"/"</span><span class="token punctuation">)</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token annotation punctuation">@GetMapping</span><span class="token punctuation">(</span><span class="token string">"/all-cookies"</span><span class="token punctuation">)</span>
...
<p><img src="@source/docs/system-design/security/images/basis-of-authority-certification/session-cookie.png" alt=""></p>
<ol>
...
<div class="language-html line-numbers-mode" data-ext="html"><pre v-pre class="language-html"><code>&lt;a src=http://www.mybank.com/Transfer?bankId=11&amp;money=10000>科学理财，年盈利率过万&lt;/>
...
<p>需要注意的是不论是 <code v-pre>Cookie</code> 还是 <code v-pre>Token</code> 都无法避免 <strong>跨站脚本攻击（Cross Site Scripting）XSS</strong> 。</p>
<blockquote>
...
<p>有没有一种不需要自己存放 <code v-pre>Session</code> 信息就能实现身份验证的方式呢？使用 <code v-pre>Token</code> 即可！<strong>JWT</strong> （JSON Web Token） 就是这种方式的实现，通过这种方式服务器端就不需要保存 <code v-pre>Session</code> 数据了，只用在客户端保存服务端返回给客户的 <code v-pre>Token</code> 就可以了，扩展性得到提升。</p>
...
<li><strong>Signature（签名）</strong> ：服务器通过<code v-pre>Payload</code>、<code v-pre>Header</code>和一个密钥(<code v-pre>secret</code>)使用 <code v-pre>Header</code> 里面指定的签名算法（默认是 HMAC SHA256）生成。</li>
</ol>
<h2 id="如何基于-token-进行身份验证" tabindex="-1"><a class="header-anchor" href="#如何基于-token-进行身份验证" aria-hidden="true">#</a> 如何基于 Token 进行身份验证？</h2>
<p>在基于 Token 进行身份验证的的应用程序中，服务器通过<code v-pre>Payload</code>、<code v-pre>Header</code>和一个密钥(<code v-pre>secret</code>)创建令牌（<code v-pre>Token</code>）并将 <code v-pre>Token</code> 发送给客户端，客户端将 <code v-pre>Token</code> 保存在 Cookie 或者 localStorage 里面，以后客户端发出的所有请求都会携带这个令牌。你可以把它放在 Cookie 里面自动发送，但是这样不能跨域，所以更好的做法是放在 HTTP Header 的 Authorization 字段中：<code v-pre>Authorization: Bearer Token</code>。</p>
<p><img src="@source/docs/system-design/security/images/basis-of-authority-certification/jwt.png" alt="jwt"></p>
<ol>
<li>用户向服务器发送用户名和密码用于登陆系统。</li>
<li>身份验证服务响应并返回了签名的 JWT，上面包含了用户是谁的内容。</li>
<li>用户以后每次向后端发请求都在 <code v-pre>Header</code> 中带上 JWT。</li>
...
<p><img src="@source/docs/system-design/security/images/basis-of-authority-certification/sso.png" alt="sso"></p>
...
<p>微信支付账户相关参数：</p>
<p><img src="@source/docs/system-design/security/images/basis-of-authority-certification/微信支付-fnglfdlgdfj.png" alt=""></p>
