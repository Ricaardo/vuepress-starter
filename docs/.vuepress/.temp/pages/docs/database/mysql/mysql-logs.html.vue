...
<p>它们不都是刷盘么？差别在哪里？</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token number">1</span> <span class="token class-name">Byte</span> <span class="token operator">=</span> <span class="token number">8</span>bit
...
<p>虽然性能得到提升，但是机器宕机，<code v-pre>page cache</code>里面的 binglog 会丢失。</p>
<p>为了安全起见，可以设置为<code v-pre>1</code>，表示每次提交事务都会执行<code v-pre>fsync</code>，就如同<strong>binlog 日志刷盘流程</strong>一样。</p>
