<template><div><h1 id="idea源码阅读技巧" tabindex="-1"><a class="header-anchor" href="#idea源码阅读技巧" aria-hidden="true">#</a> IDEA源码阅读技巧</h1>
...
<tr>
<td>⭐⭐⭐⭐⭐</td>
<td><code v-pre>Ctrl + H</code></td>
</tr>
</tbody>
</table>
<p>平时，我们阅读源码的时候，经常需要查看类的层次结构。就比如我们遇到抽象类或者接口的时候，经常需要查看其被哪些类实现。</p>
<p>拿 Spring 源码为例，<code v-pre>BeanDefinition</code> 是一个关于 Bean 属性/定义的接口。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果我们需要查看 <code v-pre>BeanDefinition</code> 被哪些类实现的话，只需要把鼠标移动到 <code v-pre>BeanDefinition</code> 类名上，然后使用快捷键 <code v-pre>Ctrl + H</code> 即可。</p>
<p><img src="https://guide-blog-images.oss-cn-shenzhen.aliyuncs.com/idea/20210527135533686.png" alt=""></p>
<p>同理，如果你想查看接口 <code v-pre>BeanDefinition</code> 继承的接口 <code v-pre>AttributeAccessor</code> 被哪些类实现的话，只需要把鼠标移动到 <code v-pre>AttributeAccessor</code> 类名上，然后使用快捷键 <code v-pre>Ctrl + H</code> 即可。</p>
...
<tr>
<td>⭐⭐⭐⭐</td>
<td><code v-pre>Alt + 7</code>(Win) / <code v-pre>Command +7</code> （Mac）</td>
</tr>
</tbody>
</table>
<p>类结构可以让我们快速了解到当前类的方法、变量/常量，非常使用！</p>
<p>我们在对应的类的任意位置使用快捷键 <code v-pre>Alt + 7</code>(Win) / <code v-pre>Command +7</code> （Mac）即可。</p>
...
<tr>
<td>⭐⭐⭐⭐⭐</td>
<td><code v-pre>Ctrl + N</code> (Win) / <code v-pre>Command + O</code> （Mac）</td>
</tr>
</tbody>
</table>
<p>使用快捷键 <code v-pre>Ctrl + N</code> (Win) / <code v-pre>Command + O</code> （Mac）可以快速检索类/文件。</p>
...
<ul>
<li>当前文件下检索 ： <code v-pre>Ctrl + F</code> (Win) / <code v-pre>Command + F</code> （Mac）</li>
<li>全局的文本检索 : <code v-pre>Ctrl + Shift + F</code> (Win) / <code v-pre>Command + Shift + F</code> （Mac）</li>
</ul>
...
<tr>
<td>⭐⭐⭐⭐</td>
<td><code v-pre>Ctrl + Alt + B</code> (Win) / <code v-pre>Command + Alt + B</code> (Mac)</td>
</tr>
</tbody>
</table>
<p>如果我们想直接跳转到某个方法/类的实现类，直接在方法名或者类名上使用快捷键 <code v-pre>Ctrl + Alt + B/鼠标左键</code> (Win) / <code v-pre>Command + Alt + B/鼠标左键</code> (Mac) 即可。</p>
<p>如果对应的方法/类只有一个实现类的话，会直接跳转到对应的实现类。</p>
<p>比如 <code v-pre>BeanDefinition</code> 接口的 <code v-pre>getBeanClassName()</code> 方法只被 <code v-pre>AbstractBeanDefinition</code> 抽象类实现，我们对这个方法使用快捷键就可以直接跳转到 <code v-pre>AbstractBeanDefinition</code> 抽象类中对应的实现方法。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">BeanDefinition</span> <span class="token keyword">extends</span> <span class="token class-name">AttributeAccessor</span><span class="token punctuation">,</span> <span class="token class-name">BeanMetadataElement</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span><span class="token punctuation">.</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果对应的方法/类有多个实现类的话，IDEA 会弹出一个选择框让你选择。</p>
<p>比如 <code v-pre>BeanDefinition</code> 接口的 <code v-pre>getParentName()</code> 方法就有多个不同的实现。</p>
...
<tr>
<td>⭐⭐⭐⭐</td>
<td><code v-pre>Alt + F7</code></td>
</tr>
</tbody>
</table>
<p>我们可以通过直接在方法名上使用快捷键 <code v-pre>Alt + F7</code> 来查看这个方法在哪些地方被调用过。</p>
...
<tr>
<td>⭐⭐⭐⭐⭐</td>
<td><code v-pre>Ctrl + E</code>(Win) / <code v-pre>Command +E</code> （Mac）</td>
</tr>
</tbody>
</table>
<p>你可以通过快捷键 <code v-pre>Ctrl + E</code>(Win) / <code v-pre>Command +E</code> （Mac）来显示 IDEA 最近使用的一些文件。</p>
