...
<h1 id="第十三章-函数式编程" tabindex="-1"><a class="header-anchor" href="#第十三章-函数式编程" aria-hidden="true">#</a> 第十三章 函数式编程</h1>
<blockquote>
<p>函数式编程语言操纵代码片段就像操作数据一样容易。 虽然 Java 不是函数式语言，但 Java 8 Lambda 表达式和方法引用 (Method References) 允许你以函数式编程。</p>
</blockquote>
<p>在计算机时代早期，内存是稀缺和昂贵的。几乎每个人都用汇编语言编程。人们虽然知道编译器，但编译器生成的代码很低效，比手工编码的汇编程序多很多字节，仅仅想到这一点，人们还是选择汇编语言。</p>
<p>通常，为了使程序能在有限的内存上运行，在程序运行时，程序员通过修改内存中的代码，使程序可以执行不同的操作，用这种方式来节省代码空间。这种技术被称为<strong>自修改代码</strong> （self-modifying code）。只要程序小到几个人就能够维护所有棘手和难懂的汇编代码，你就能让程序运行起来。</p>
<p>随着内存和处理器变得更便宜、更快。C 语言出现并被大多数汇编程序员认为更“高级”。人们发现使用 C 可以显著提高生产力。同时，使用 C 创建自修改代码仍然不难。</p>
<p>随着硬件越来越便宜，程序的规模和复杂性都在增长。这一切只是让程序工作变得困难。我们想方设法使代码更加一致和易懂。使用纯粹的自修改代码造成的结果就是：我们很难确定程序在做什么。它也难以测试：除非你想一点点测试输出，代码转换和修改等等过程？</p>
<p>然而，使用代码以某种方式操纵其他代码的想法也很有趣，只要能保证它更安全。从代码创建，维护和可靠性的角度来看，这个想法非常吸引人。我们不用从头开始编写大量代码，而是从易于理解、充分测试及可靠的现有小块开始，最后将它们组合在一起以创建新代码。难道这不会让我们更有效率，同时创造更健壮的代码吗？</p>
<p>这就是<strong>函数式编程</strong>（FP）的意义所在。通过合并现有代码来生成新功能而不是从头开始编写所有内容，我们可以更快地获得更可靠的代码。至少在某些情况下，这套理论似乎很有用。在这一过程中，函数式语言已经产生了优雅的语法，这些语法对于非函数式语言也适用。</p>
<p>你也可以这样想：</p>
...
<p>需要提醒大家的是，函数式语言背后有很多动机，这意味着描述它们可能会有些混淆。它通常取决于各种观点：为“并行编程”，“代码可靠性”和“代码创建和库复用”。<a href="%E5%8A%9F%E8%83%BD%E7%B2%98%E8%B4%B4%E5%9C%A8%E4%B8%80%E8%B5%B7%E7%9A%84%E6%96%B9%E6%B3%95%E7%9A%84%E7%A1%AE%E6%9C%89%E7%82%B9%E4%B8%8E%E4%BC%97%E4%B8%8D%E5%90%8C%EF%BC%8C%E4%BD%86%E5%AE%83%E4%BB%8D%E4%B8%8D%E5%A4%B1%E4%B8%BA%E4%B8%80%E4%B8%AA%E5%BA%93%E3%80%82">^1</a> 关于函数式编程能高效创建更健壮的代码这一观点仍存在部分争议。虽然已有一些好的范例[^2]，但还不足以证明纯函数式语言就是解决编程问题的最佳方法。</p>
<p>FP 思想值得融入非 FP 语言，如 Python。Java 8 也从中吸收并支持了 FP。我们将在此章探讨。</p>
<!-- Old vs. New -->
<h2 id="新旧对比" tabindex="-1"><a class="header-anchor" href="#新旧对比" aria-hidden="true">#</a> 新旧对比</h2>
<p>通常，传递给方法的数据不同，结果不同。如果我们希望方法在调用时行为不同，该怎么做呢？结论是：只要能将代码传递给方法，我们就可以控制它的行为。此前，我们通过在方法中创建包含所需行为的对象，然后将该对象传递给我们想要控制的方法来完成此操作。下面我们用传统形式和 Java 8 的方法引用、Lambda 表达式分别演示。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Strategize.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>hello there?
HELLO THERE!
Hello
Hello there Hello there
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>Strategy</strong> 接口提供了单一的 <code v-pre>approach()</code> 方法来承载函数式功能。通过创建不同的 <strong>Strategy</strong> 对象，我们可以创建不同的行为。</p>
<p>我们一般通过创建一个实现<strong>Strategy</strong>接口的类来实现这种行为，正如在<strong>Soft</strong>里所做的。</p>
<ul>
<li>
<p><strong>[1]</strong> 在 <strong>Strategize</strong> 中，你可以看到 <strong>Soft</strong> 作为默认策略，在构造函数中赋值。</p>
</li>
<li>
<p><strong>[2]</strong> 一种较为简洁且更加自然的方法是创建一个<strong>匿名内部类</strong>。即便如此，仍有相当数量的冗余代码。你总需要仔细观察后才会发现：“哦，我明白了，原来这里使用了匿名内部类。”</p>
</li>
<li>
<p><strong>[3]</strong> Java 8 的 Lambda 表达式，其参数和函数体被箭头 <code v-pre>-&gt;</code> 分隔开。箭头右侧是从 Lambda 返回的表达式。它与单独定义类和采用匿名内部类是等价的，但代码少得多。</p>
</li>
<li>
<p><strong>[4]</strong> Java 8 的<strong>方法引用</strong>，它以 <code v-pre>::</code> 为特征。 <code v-pre>::</code> 的左边是类或对象的名称， <code v-pre>::</code> 的右边是方法的名称，但是没有参数列表。</p>
</li>
<li>
<p><strong>[5]</strong> 在使用默认的 <strong>Soft</strong>  策略之后，我们逐步遍历数组中的所有 <strong>Strategy</strong>，并通过调用 <code v-pre>changeStrategy()</code> 方法将每个 <strong>Strategy</strong> 传入变量 <code v-pre>s</code> 中。</p>
</li>
<li>
<p><strong>[6]</strong> 现在，每次调用 <code v-pre>communicate()</code> 都会产生不同的行为，具体取决于此刻正在使用的策略<strong>代码对象</strong>。我们传递的是行为，而并不仅仅是数据。[^3]</p>
</li>
</ul>
<p>在 Java 8 之前，我们能够通过 <strong>[1]</strong> 和 <strong>[2]</strong> 的方式传递功能。然而，这种语法的读写非常笨拙，并且我们别无选择。方法引用和 Lambda 表达式的出现让我们可以在需要时<strong>传递功能</strong>，而不是仅在必要时才这么做。</p>
<!-- Lambda Expressions -->
<h2 id="lambda表达式" tabindex="-1"><a class="header-anchor" href="#lambda表达式" aria-hidden="true">#</a> Lambda表达式</h2>
<p>Lambda 表达式是使用<strong>最小可能</strong>语法编写的函数定义：</p>
<ol>
<li>
<p>Lambda 表达式产生函数，而不是类。 虽然在 JVM（Java Virtual Machine，Java 虚拟机）上，一切都是类，但是幕后有各种操作执行让 Lambda 看起来像函数 —— 作为程序员，你可以高兴地假装它们“就是函数”。</p>
</li>
<li>
<p>Lambda 语法尽可能少，这正是为了使 Lambda 易于编写和使用。</p>
</li>
</ol>
<p>我们在 <strong>Strategize.java</strong> 中看到了一个 Lambda 表达式，但还有其他语法变体：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/LambdaExpressions.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Oh! No Parens!
Hi! More details
Short info
Pi! 3.14159
moreLines()
from moreLines()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们从三个接口开始，每个接口都有一个单独的方法（很快就会理解它的重要性）。但是，每个方法都有不同数量的参数，以便演示 Lambda 表达式语法。</p>
...
<li>
<p>接着 <code v-pre>-&gt;</code>，可视为“产出”。</p>
</li>
<li>
<p><code v-pre>-&gt;</code> 之后的内容都是方法体。</p>
</li>
</ol>
<ul>
<li>
<p><strong>[1]</strong> 当只用一个参数，可以不需要括号 <code v-pre>()</code>。 然而，这是一个特例。</p>
</li>
<li>
<p><strong>[2]</strong> 正常情况使用括号 <code v-pre>()</code> 包裹参数。 为了保持一致性，也可以使用括号 <code v-pre>()</code> 包裹单个参数，虽然这种情况并不常见。</p>
</li>
<li>
<p><strong>[3]</strong> 如果没有参数，则必须使用括号 <code v-pre>()</code> 表示空参数列表。</p>
</li>
<li>
<p><strong>[4]</strong> 对于多个参数，将参数列表放在括号 <code v-pre>()</code> 中。</p>
</li>
</ul>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/IntCall.java</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>整数 n 的阶乘将所有小于或等于 n 的正整数相乘。 阶乘函数是一个常见的递归示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/RecursiveFactorial.java</span>
...
362880
3628800
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里，<code v-pre>fact</code> 是一个静态变量。 注意使用三元 <strong>if-else</strong>。 递归函数将一直调用自己，直到 <code v-pre>i == 0</code>。所有递归函数都有“停止条件”，否则将无限递归并产生异常。</p>
<p>我们可以将 <code v-pre>Fibonacci</code> 序列用递归的 Lambda 表达式来实现，这次使用实例变量：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/RecursiveFibonacci.java</span>
...
34
55
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>将 <code v-pre>Fibonacci</code> 序列中的最后两个元素求和来产生下一个元素。</p>
<!-- method references-->
<h2 id="方法引用" tabindex="-1"><a class="header-anchor" href="#方法引用" aria-hidden="true">#</a> 方法引用</h2>
<p>Java 8 方法引用没有历史包袱。方法引用组成：类名或对象名，后面跟 <code v-pre>::</code> [^4]，然后跟方法名称。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/MethodReferences.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>call()
Hello, Bob
valuable information
Help!
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>[1]</strong> 我们从单一方法接口开始（同样，你很快就会了解到这一点的重要性）。</p>
<p><strong>[2]</strong> <code v-pre>show()</code> 的签名（参数类型和返回类型）符合 <strong>Callable</strong> 的 <code v-pre>call()</code> 的签名。</p>
<p><strong>[3]</strong> <code v-pre>hello()</code> 也符合 <code v-pre>call()</code> 的签名。</p>
<p><strong>[4]</strong>  <code v-pre>help()</code> 也符合，它是静态内部类中的非静态方法。</p>
<p><strong>[5]</strong> <code v-pre>assist()</code> 是静态内部类中的静态方法。</p>
<p><strong>[6]</strong> 我们将 <strong>Describe</strong> 对象的方法引用赋值给 <strong>Callable</strong> ，它没有 <code v-pre>show()</code> 方法，而是 <code v-pre>call()</code> 方法。 但是，Java 似乎接受用这个看似奇怪的赋值，因为方法引用符合 <strong>Callable</strong> 的 <code v-pre>call()</code> 方法的签名。</p>
<p><strong>[7]</strong> 我们现在可以通过调用 <code v-pre>call()</code> 来调用 <code v-pre>show()</code>，因为 Java 将 <code v-pre>call()</code> 映射到 <code v-pre>show()</code>。</p>
...
<h3 id="runnable接口" tabindex="-1"><a class="header-anchor" href="#runnable接口" aria-hidden="true">#</a> Runnable接口</h3>
<p><strong>Runnable</strong> 接口自 1.0 版以来一直在 Java 中，因此不需要导入。它也符合特殊的单方法接口格式：它的方法 <code v-pre>run()</code> 不带参数，也没有返回值。因此，我们可以使用 Lambda 表达式和方法引用作为 <strong>Runnable</strong>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/RunnableMethodReference.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Anonymous
lambda
Go::go()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>Thread</strong> 对象将 <strong>Runnable</strong> 作为其构造函数参数，并具有会调用 <code v-pre>run()</code> 的方法  <code v-pre>start()</code>。 注意这里只有<strong>匿名内部类</strong>才要求显式声明 <code v-pre>run()</code> 方法。</p>
<!-- Unbound Method References -->
<h3 id="未绑定的方法引用" tabindex="-1"><a class="header-anchor" href="#未绑定的方法引用" aria-hidden="true">#</a> 未绑定的方法引用</h3>
<p>未绑定的方法引用是指没有关联对象的普通（非静态）方法。 使用未绑定的引用时，我们必须先提供对象：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/UnboundMethodReference.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>X::f()
X::f()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>到目前为止，我们已经见过了方法引用和对应接口的签名（参数类型和返回类型）一致的几个赋值例子。 在 <strong>[1]</strong> 中，我们尝试同样的做法，把 <code v-pre>X</code> 的 <code v-pre>f()</code> 方法引用赋值给 <strong>MakeString</strong>。结果即使 <code v-pre>make()</code> 与 <code v-pre>f()</code> 具有相同的签名，编译也会报“invalid method reference”（无效方法引用）错误。 问题在于，这里其实还需要另一个隐藏参数参与：我们的老朋友 <code v-pre>this</code>。 你不能在没有 <code v-pre>X</code> 对象的前提下调用 <code v-pre>f()</code>。 因此，<code v-pre>X :: f</code> 表示未绑定的方法引用，因为它尚未“绑定”到对象。</p>
<p>要解决这个问题，我们需要一个 <code v-pre>X</code> 对象，因此我们的接口实际上需要一个额外的参数，正如在 <strong>TransformX</strong> 中看到的那样。 如果将 <code v-pre>X :: f</code> 赋值给 <strong>TransformX</strong>，在 Java 中是允许的。我们必须做第二个心理调整——使用未绑定的引用时，函数式方法的签名（接口中的单个方法）不再与方法引用的签名完全匹配。 原因是：你需要一个对象来调用方法。</p>
<p><strong>[2]</strong> 的结果有点像脑筋急转弯。我拿到未绑定的方法引用，并且调用它的<code v-pre>transform()</code>方法，将一个X类的对象传递给它，最后使得 <code v-pre>x.f()</code> 以某种方式被调用。Java知道它必须拿第一个参数，该参数实际就是<code v-pre>this</code> 对象，然后对此调用方法。</p>
<p>如果你的方法有更多个参数，就以第一个参数接受<code v-pre>this</code>的模式来处理。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/MultiUnbound.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">MultiUnbound</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>需要指出的是，我将类命名为 <strong>This</strong>，并将函数式方法的第一个参数命名为 <strong>athis</strong>，但你在生产级代码中应该使用其他名字，以防止混淆。</p>
<h3 id="构造函数引用" tabindex="-1"><a class="header-anchor" href="#构造函数引用" aria-hidden="true">#</a> 构造函数引用</h3>
<p>你还可以捕获构造函数的引用，然后通过引用调用该构造函数。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/CtorReference.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">CtorReference</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>Dog</strong> 有三个构造函数，函数式接口内的 <code v-pre>make()</code> 方法反映了构造函数参数列表（ <code v-pre>make()</code> 方法名称可以不同）。</p>
<p><strong>注意</strong>我们如何对 <strong>[1]</strong>，<strong>[2]</strong> 和 <strong>[3]</strong> 中的每一个使用 <code v-pre>Dog :: new</code>。 这三个构造函数只有一个相同名称：<code v-pre>:: new</code>，但在每种情况下赋值给不同的接口，编译器可以从中知道具体使用哪个构造函数。</p>
<p>编译器知道调用函数式方法（本例中为 <code v-pre>make()</code>）就相当于调用构造函数。</p>
<!-- Functional Interfaces -->
<h2 id="函数式接口" tabindex="-1"><a class="header-anchor" href="#函数式接口" aria-hidden="true">#</a> 函数式接口</h2>
<p>方法引用和 Lambda 表达式都必须被赋值，同时赋值需要类型信息才能使编译器保证类型的正确性。尤其是Lambda 表达式，它引入了新的要求。 代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>x <span class="token operator">-></span> x<span class="token punctuation">.</span><span class="token function">toString</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>我们清楚这里返回类型必须是 <strong>String</strong>，但 <code v-pre>x</code> 是什么类型呢？</p>
<p>Lambda 表达式包含 <em>类型推导</em> （编译器会自动推导出类型信息，避免了程序员显式地声明）。编译器必须能够以某种方式推导出 <code v-pre>x</code> 的类型。</p>
<p>下面是第二个代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token punctuation">(</span>x<span class="token punctuation">,</span> y<span class="token punctuation">)</span> <span class="token operator">-></span> x <span class="token operator">+</span> y
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>现在 <code v-pre>x</code> 和 <code v-pre>y</code> 可以是任何支持 <code v-pre>+</code> 运算符连接的数据类型，可以是两个不同的数值类型或者是 一个 <strong>String</strong> 加任意一种可自动转换为 <strong>String</strong> 的数据类型（这包括了大多数类型）。 但是，当 Lambda 表达式被赋值时，编译器必须确定 <code v-pre>x</code> 和 <code v-pre>y</code> 的确切类型以生成正确的代码。</p>
<p>该问题也适用于方法引用。 假设你要传递 <code v-pre>System.out :: println</code> 到你正在编写的方法 ，你怎么知道传递给方法的参数的类型？</p>
<p>为了解决这个问题，Java 8 引入了 <code v-pre>java.util.function</code> 包。它包含一组接口，这些接口是 Lambda 表达式和方法引用的目标类型。 每个接口只包含一个抽象方法，称为 <em>函数式方法</em> 。</p>
<p>在编写接口时，可以使用 <code v-pre>@FunctionalInterface</code> 注解强制执行此“函数式方法”模式：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/FunctionalAnnotation.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">FunctionalAnnotation</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>@FunctionalInterface</code> 注解是可选的; Java 会在 <code v-pre>main()</code> 中把 <strong>Functional</strong> 和 <strong>FunctionalNoAnn</strong> 都当作函数式接口来看待。 在 <code v-pre>NotFunctional</code> 的定义中可看出<code v-pre>@FunctionalInterface</code> 的作用：当接口中抽象方法多于一个时产生编译期错误。</p>
<p>仔细观察在定义 <code v-pre>f</code> 和 <code v-pre>fna</code> 时发生了什么。 <code v-pre>Functional</code> 和 <code v-pre>FunctionalNoAnn</code> 声明了是接口，然而被赋值的只是方法 <code v-pre>goodbye()</code>。首先，这只是一个方法而不是类；其次，它甚至都不是实现了该接口的类中的方法。这是添加到Java 8中的一点小魔法：如果将方法引用或 Lambda 表达式赋值给函数式接口（类型需要匹配），Java 会适配你的赋值到目标接口。 编译器会在后台把方法引用或 Lambda 表达式包装进实现目标接口的类的实例中。</p>
<p>虽然 <code v-pre>FunctionalAnnotation</code> 确实符合 <code v-pre>Functional</code> 模型，但是 Java不允许我们像<code v-pre>fac</code>定义的那样，将 <code v-pre>FunctionalAnnotation</code> 直接赋值给 <code v-pre>Functional</code>，因为 <code v-pre>FunctionalAnnotation</code> 并没有显式地去实现 <code v-pre>Functional</code> 接口。唯一的惊喜是，Java 8 允许我们将函数赋值给接口，这样的语法更加简单漂亮。</p>
<p><code v-pre>java.util.function</code> 包旨在创建一组完整的目标接口，使得我们一般情况下不需再定义自己的接口。主要因为基本类型的存在，导致预定义的接口数量有少许增加。 如果你了解命名模式，顾名思义就能知道特定接口的作用。</p>
<p>以下是基本命名准则：</p>
<ol>
<li>
<p>如果只处理对象而非基本类型，名称则为 <code v-pre>Function</code>，<code v-pre>Consumer</code>，<code v-pre>Predicate</code> 等。参数类型通过泛型添加。</p>
</li>
<li>
<p>如果接收的参数是基本类型，则由名称的第一部分表示，如 <code v-pre>LongConsumer</code>，<code v-pre>DoubleFunction</code>，<code v-pre>IntPredicate</code> 等，但返回基本类型的 <code v-pre>Supplier</code> 接口例外。</p>
</li>
<li>
<p>如果返回值为基本类型，则用 <code v-pre>To</code> 表示，如 <code v-pre>ToLongFunction &lt;T&gt;</code> 和 <code v-pre>IntToLongFunction</code>。</p>
</li>
<li>
<p>如果返回值类型与参数类型相同，则是一个 <code v-pre>Operator</code> ：单个参数使用 <code v-pre>UnaryOperator</code>，两个参数使用 <code v-pre>BinaryOperator</code>。</p>
</li>
<li>
<p>如果接收参数并返回一个布尔值，则是一个 <strong>谓词</strong> (<code v-pre>Predicate</code>)。</p>
</li>
<li>
<p>如果接收的两个参数类型不同，则名称中有一个 <code v-pre>Bi</code>。</p>
</li>
</ol>
<p>下表描述了 <code v-pre>java.util.function</code> 中的目标类型（包括例外情况）：</p>
<table>
<thead>
<tr>
...
<td style="text-align:center"><strong>Runnable</strong> <br> (java.lang)  <br>  <code v-pre>run()</code></td>
<td style="text-align:center"><strong>Runnable</strong></td>
</tr>
<tr>
<td style="text-align:left">无参数； <br> 返回类型任意</td>
<td style="text-align:center"><strong>Supplier</strong> <br> <code v-pre>get()</code> <br> <code v-pre>getAs类型()</code></td>
<td style="text-align:center"><strong>Supplier<code v-pre>&lt;T&gt;</code>  <br> BooleanSupplier  <br> IntSupplier  <br> LongSupplier  <br> DoubleSupplier</strong></td>
</tr>
<tr>
<td style="text-align:left">无参数； <br> 返回类型任意</td>
<td style="text-align:center"><strong>Callable</strong> <br> (java.util.concurrent)  <br> <code v-pre>call()</code></td>
<td style="text-align:center"><strong>Callable<code v-pre>&lt;V&gt;</code></strong></td>
</tr>
<tr>
<td style="text-align:left">1 参数； <br> 无返回值</td>
<td style="text-align:center"><strong>Consumer</strong> <br> <code v-pre>accept()</code></td>
<td style="text-align:center"><strong><code v-pre>Consumer&lt;T&gt;</code> <br> IntConsumer <br> LongConsumer <br> DoubleConsumer</strong></td>
</tr>
<tr>
<td style="text-align:left">2 参数 <strong>Consumer</strong></td>
<td style="text-align:center"><strong>BiConsumer</strong> <br> <code v-pre>accept()</code></td>
<td style="text-align:center"><strong><code v-pre>BiConsumer&lt;T,U&gt;</code></strong></td>
</tr>
<tr>
<td style="text-align:left">2 参数 <strong>Consumer</strong>； <br> 第一个参数是 引用； <br> 第二个参数是 基本类型</td>
<td style="text-align:center"><strong>Obj类型Consumer</strong> <br> <code v-pre>accept()</code></td>
<td style="text-align:center"><strong><code v-pre>ObjIntConsumer&lt;T&gt;</code> <br> <code v-pre>ObjLongConsumer&lt;T&gt;</code> <br> <code v-pre>ObjDoubleConsumer&lt;T&gt;</code></strong></td>
</tr>
<tr>
<td style="text-align:left">1 参数； <br> 返回类型不同</td>
<td style="text-align:center"><strong>Function</strong> <br> <code v-pre>apply()</code> <br> <strong>To类型</strong> 和 <strong>类型To类型</strong> <br> <code v-pre>applyAs类型()</code></td>
<td style="text-align:center"><strong>Function<code v-pre>&lt;T,R&gt;</code> <br> IntFunction<code v-pre>&lt;R&gt;</code> <br> <code v-pre>LongFunction&lt;R&gt;</code> <br> DoubleFunction<code v-pre>&lt;R&gt;</code> <br> ToIntFunction<code v-pre>&lt;T&gt;</code> <br> <code v-pre>ToLongFunction&lt;T&gt;</code> <br> <code v-pre>ToDoubleFunction&lt;T&gt;</code> <br> IntToLongFunction <br> IntToDoubleFunction <br> LongToIntFunction <br> LongToDoubleFunction <br> DoubleToIntFunction <br> DoubleToLongFunction</strong></td>
</tr>
<tr>
<td style="text-align:left">1 参数； <br> 返回类型相同</td>
<td style="text-align:center"><strong>UnaryOperator</strong> <br> <code v-pre>apply()</code></td>
<td style="text-align:center"><strong><code v-pre>UnaryOperator&lt;T&gt;</code> <br> IntUnaryOperator <br> LongUnaryOperator <br> DoubleUnaryOperator</strong></td>
</tr>
<tr>
<td style="text-align:left">2 参数，类型相同； <br> 返回类型相同</td>
<td style="text-align:center"><strong>BinaryOperator</strong> <br> <code v-pre>apply()</code></td>
<td style="text-align:center"><strong><code v-pre>BinaryOperator&lt;T&gt;</code> <br> IntBinaryOperator <br> LongBinaryOperator <br> DoubleBinaryOperator</strong></td>
</tr>
<tr>
<td style="text-align:left">2 参数，类型相同; <br> 返回整型</td>
<td style="text-align:center">Comparator <br> (java.util) <br> <code v-pre>compare()</code></td>
<td style="text-align:center"><strong><code v-pre>Comparator&lt;T&gt;</code></strong></td>
</tr>
<tr>
<td style="text-align:left">2 参数； <br> 返回布尔型</td>
<td style="text-align:center"><strong>Predicate</strong> <br> <code v-pre>test()</code></td>
<td style="text-align:center"><strong><code v-pre>Predicate&lt;T&gt;</code> <br> <code v-pre>BiPredicate&lt;T,U&gt;</code> <br> IntPredicate <br> LongPredicate <br> DoublePredicate</strong></td>
</tr>
<tr>
<td style="text-align:left">参数基本类型； <br> 返回基本类型</td>
<td style="text-align:center"><strong>类型To类型Function</strong> <br> <code v-pre>applyAs类型()</code></td>
...
<tr>
<td style="text-align:left">2 参数； <br>类型不同</td>
<td style="text-align:center"><strong>Bi操作</strong> <br> (不同方法名)</td>
<td style="text-align:center"><strong><code v-pre>BiFunction&lt;T,U,R&gt;</code> <br> <code v-pre>BiConsumer&lt;T,U&gt;</code> <br> <code v-pre>BiPredicate&lt;T,U&gt;</code> <br> <code v-pre>ToIntBiFunction&lt;T,U&gt;</code> <br> <code v-pre>ToLongBiFunction&lt;T,U&gt;</code> <br> <code v-pre>ToDoubleBiFunction&lt;T&gt;</code></strong></td>
</tr>
</tbody>
</table>
<p>此表仅提供些常规方案。通过上表，你应该或多或少能自行推导出你所需要的函数式接口。</p>
<p>可以看出，在创建 <code v-pre>java.util.function</code> 时，设计者们做出了一些选择。</p>
<p>例如，为什么没有 <code v-pre>IntComparator</code>，<code v-pre>LongComparator</code> 和 <code v-pre>DoubleComparator</code> 呢？有 <code v-pre>BooleanSupplier</code> 却没有其他表示 <strong>Boolean</strong> 的接口；有通用的 <code v-pre>BiConsumer</code> 却没有用于 <strong>int</strong>，<strong>long</strong> 和 <strong>double</strong> 的 <code v-pre>BiConsumers</code> 变体（我理解他们为什么放弃这些接口）。这到底是疏忽还是有人认为其他组合使用得很少呢（他们是如何得出这个结论的）？</p>
<p>你还可以看到基本类型给 Java 添加了多少复杂性。该语言的第一版中就包含了基本类型，原因是考虑效率问题（该问题很快就缓解了）。现在，在语言的生命周期里，我们一直忍受语言设计的糟糕选择所带来的影响。</p>
<p>下面枚举了基于 Lambda 表达式的所有不同 <strong>Function</strong> 变体的示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/FunctionVariants.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">FunctionVariants</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这些 Lambda 表达式尝试生成适合函数签名的最简代码。 在某些情况下有必要进行强制类型转换，否则编译器会报截断错误。</p>
<p><code v-pre>main()</code>中的每个测试都显示了 <code v-pre>Function</code> 接口中不同类型的 <code v-pre>apply()</code> 方法。 每个都产生一个与其关联的 Lambda 表达式的调用。</p>
<p>方法引用有自己的小魔法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token operator">/</span> functional<span class="token operator">/</span><span class="token class-name">MethodConversion</span><span class="token punctuation">.</span>java
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>accept()
someOtherName()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>查看 <code v-pre>BiConsumer</code> 的文档，你会看到它的函数式方法为 <code v-pre>accept()</code> 。 的确，如果我们将方法命名为 <code v-pre>accept()</code>，它就可以作为方法引用。 但是我们也可用不同的名称，比如 <code v-pre>someOtherName()</code>。只要参数类型、返回类型与 <code v-pre>BiConsumer</code> 的 <code v-pre>accept()</code> 相同即可。</p>
<p>因此，在使用函数接口时，名称无关紧要——只要参数类型和返回类型相同。 Java 会将你的方法映射到接口方法。 要调用方法，可以调用接口的函数式方法名（在本例中为 <code v-pre>accept()</code>），而不是你的方法名。</p>
<p>现在我们来看看，将方法引用应用于基于类的函数式接口（即那些不包含基本类型的函数式接口）。下面的例子中，我创建了适合函数式方法签名的最简单的方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/ClassFunctionals.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ClassFunctionals</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>请<strong>注意</strong>，每个方法名称都是随意的（如 <code v-pre>f1()</code>，<code v-pre>f2()</code>等）。正如你刚才看到的，一旦将方法引用赋值给函数接口，我们就可以调用与该接口关联的函数方法。 在此示例中为 <code v-pre>get()</code>、<code v-pre>compare()</code>、<code v-pre>accept()</code>、<code v-pre>apply()</code> 和 <code v-pre>test()</code>。</p>
<!-- Functional Interfaces with More Arguments -->
<h3 id="多参数函数式接口" tabindex="-1"><a class="header-anchor" href="#多参数函数式接口" aria-hidden="true">#</a> 多参数函数式接口</h3>
<p><code v-pre>java.util.functional</code> 中的接口是有限的。比如有 <code v-pre>BiFunction</code>，但也仅此而已。 如果需要三参数函数的接口怎么办？ 其实这些接口非常简单，很容易查看 Java 库源代码并自行创建。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/TriFunction.java</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>简单测试，验证它是否有效：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/TriFunctionTest.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TriFunctionTest</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里我们同时测试了方法引用和 Lambda 表达式。</p>
<h3 id="缺少基本类型的函数" tabindex="-1"><a class="header-anchor" href="#缺少基本类型的函数" aria-hidden="true">#</a> 缺少基本类型的函数</h3>
<p>让我们重温一下 <code v-pre>BiConsumer</code>，看看我们将如何创建各种缺失的预定义组合，涉及 <strong>int</strong>，<strong>long</strong> 和 <strong>double</strong> （基本类型）：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/BiConsumerPermutations.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>47, 11.340000
92, 22.450000
1, 11
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里使用 <code v-pre>System.out.format()</code> 来显示。它类似于 <code v-pre>System.out.println()</code> 但提供了更多的显示选项。 这里，<code v-pre>%f</code> 表示我将 <code v-pre>n</code> 作为浮点值给出，<code v-pre>%d</code> 表示 <code v-pre>n</code> 是一个整数值。 这其中可以包含空格，输入 <code v-pre>%n</code> 会换行 — 当然使用传统的 <code v-pre>\n</code> 也能换行，但 <code v-pre>%n</code> 是自动跨平台的，这是使用 <code v-pre>format()</code> 的另一个原因。</p>
<p>上例只是简单使用了合适的包装类型，而装箱和拆箱负责它与基本类型之间的来回转换。 又比如，我们可以将包装类型和<code v-pre>Function</code>一起使用，而不去用各种针对基本类型的预定义接口。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/FunctionWithWrapped.java</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为我们可以简单地写 <code v-pre>Function &lt;Integer，Double&gt;</code> 并产生正常的结果，所以用基本类型（<code v-pre>IntToDoubleFunction</code>）的唯一理由是可以避免传递参数和返回结果过程中的自动拆装箱，进而提升性能。</p>
...
<p>这个名字可能听起来令人生畏，但是：<a href="https://en.wikipedia.org/wiki/Higher-order_function" target="_blank" rel="noopener noreferrer">高阶函数<ExternalLinkIcon/></a>（Higher-order Function）只是一个消费或产生函数的函数。</p>
<p>我们先来看看如何产生一个函数：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/ProduceFunction.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>yelling
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这里，<code v-pre>produce()</code> 是高阶函数。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/ConsumeFunction.java</span>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/TransformFunction.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>I
O
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>在这里，<code v-pre>transform()</code> 生成一个与传入的函数具有相同签名的函数，但是你可以生成任何你想要的类型。</p>
<p>这里使用到了 <code v-pre>Function</code> 接口中名为 <code v-pre>andThen()</code> 的默认方法，该方法专门用于操作函数。 顾名思义，在调用 <code v-pre>in</code> 函数之后调用 <code v-pre>andThen()</code>（还有个 <code v-pre>compose()</code> 方法，它在 <code v-pre>in</code> 函数之前应用新函数）。 要附加一个 <code v-pre>andThen()</code> 函数，我们只需将该函数作为参数传递。 <code v-pre>transform()</code> 产生的是一个新函数，它将 <code v-pre>in</code> 的动作与 <code v-pre>andThen()</code> 参数的动作结合起来。</p>
<!-- Closures -->
<h2 id="闭包" tabindex="-1"><a class="header-anchor" href="#闭包" aria-hidden="true">#</a> 闭包</h2>
<p>在上一节的 <code v-pre>ProduceFunction.java</code> 中，我们从方法中返回 Lambda 函数。 虽然过程简单，但是有些问题必须再回过头来探讨一下。</p>
<p><strong>闭包</strong>（Closure）一词总结了这些问题。 它非常重要，利用闭包可以轻松生成函数。</p>
<p>考虑一个更复杂的 Lambda，它使用函数作用域之外的变量。 返回该函数会发生什么？ 也就是说，当你调用函数时，它对那些 “外部 ”变量引用了什么?  如果语言不能自动解决，那问题将变得非常棘手。 能够解决这个问题的语言被称作 <em>支持闭包</em>，或者称作 <em>词法定界</em>（<em>lexically scoped</em> ，基于词法作用域的）( 也有用术语 <em>变量捕获</em> <em>variable capture</em> 称呼的)。Java 8 提供了有限但合理的闭包支持，我们将用一些简单的例子来研究它。</p>
<p>首先，下列方法返回一个函数，该函数访问对象字段和方法参数：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure1.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure1</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但是，仔细考虑一下，<code v-pre>i</code> 的这种用法并非是个大难题，因为对象很可能在你调用 <code v-pre>makeFun()</code> 之后就存在了——实际上，垃圾收集器几乎肯定会保留以这种方式被绑定到现存函数的对象[^5]。当然，如果你对同一个对象多次调用 <code v-pre>makeFun()</code> ，你最终会得到多个函数，它们共享 <code v-pre>i</code> 的存储空间：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/SharedStorage.java</span>
...
1
2
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>每次调用 <code v-pre>getAsInt()</code> 都会增加 <code v-pre>i</code>，表明存储是共享的。</p>
<p>如果 <code v-pre>i</code> 是 <code v-pre>makeFun()</code> 的局部变量怎么办？ 在正常情况下，当 <code v-pre>makeFun()</code> 完成时 <code v-pre>i</code> 就消失。 但它仍可以编译：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure2.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure2</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>由 <code v-pre>makeFun()</code> 返回的 <code v-pre>IntSupplier</code> “关住了” <code v-pre>i</code> 和 <code v-pre>x</code>，因此即使<code v-pre>makeFun()</code>已执行完毕，当你调用返回的函数时<code v-pre>i</code> 和 <code v-pre>x</code>仍然有效，而不是像正常情况下那样在 <code v-pre>makeFun()</code> 执行后 <code v-pre>i</code> 和<code v-pre>x</code>就消失了。 但请注意，我没有像 <code v-pre>Closure1.java</code> 那样递增 <code v-pre>i</code>，因为会产生编译时错误。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure3.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure3</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>x</code> 和 <code v-pre>i</code> 的操作都犯了同样的错误：被 Lambda 表达式引用的局部变量必须是 <code v-pre>final</code> 或者是等同 <code v-pre>final</code> 效果的。</p>
<p>如果使用 <code v-pre>final</code> 修饰 <code v-pre>x</code>和 <code v-pre>i</code>，就不能再递增它们的值了。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure4.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure4</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>那么为什么在 <code v-pre>Closure2.java</code> 中， <code v-pre>x</code> 和 <code v-pre>i</code> 非 <code v-pre>final</code> 却可以运行呢？</p>
<p>这就叫做<strong>等同 final 效果</strong>（Effectively Final）。这个术语是在 Java 8 才开始出现的，表示虽然没有明确地声明变量是 <code v-pre>final</code> 的，但是因变量值没被改变过而实际有了 <code v-pre>final</code> 同等的效果。 如果局部变量的初始值永远不会改变，那么它实际上就是 <code v-pre>final</code> 的。</p>
<p>如果 <code v-pre>x</code> 和 <code v-pre>i</code> 的值在方法中的其他位置发生改变（但不在返回的函数内部），则编译器仍将视其为错误。每个递增操作则会分别产生错误消息。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure5.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure5</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>等同 final 效果</strong>意味着可以在变量声明前加上 <strong>final</strong> 关键字而不用更改任何其余代码。 实际上它就是具备 <code v-pre>final</code> 效果的，只是没有明确说明。</p>
<p>在闭包中，在使用 <code v-pre>x</code> 和 <code v-pre>i</code> 之前，通过将它们赋值给 <code v-pre>final</code> 修饰的变量，我们解决了 <code v-pre>Closure5.java</code> 中遇到的问题。代码示例：</p>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure6</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上例中 <code v-pre>iFinal</code> 和 <code v-pre>xFinal</code> 的值在赋值后并没有改变过，因此在这里使用 <code v-pre>final</code> 是多余的。</p>
<p>如果改用包装类型会是什么情况呢？我们可以把<code v-pre>int</code>类型改为<code v-pre>Integer</code>类型研究一下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure7.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure7</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>编译器非常聪明地识别到变量 <code v-pre>i</code> 的值被更改过。 包装类型可能是被特殊处理了，我们再尝试下 <strong>List</strong>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure8.java</span>
...
[1, 42]
[1, 96]
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可以看到，这次一切正常。我们改变了 <strong>List</strong> 的内容却没产生编译时错误。通过观察本例的输出结果，我们发现这看起来非常安全。这是因为每次调用 <code v-pre>makeFun()</code> 时，其实都会创建并返回一个全新而非共享的 <code v-pre>ArrayList</code>。也就是说，每个闭包都有自己独立的 <code v-pre>ArrayList</code>，它们之间互不干扰。</p>
<p>请<strong>注意</strong>我已经声明 <code v-pre>ai</code> 是 <code v-pre>final</code> 的了。尽管在这个例子中你可以去掉 <code v-pre>final</code> 并得到相同的结果（试试吧！）。 应用于对象引用的 <code v-pre>final</code> 关键字仅表示不会重新赋值引用。 它并不代表你不能修改对象本身。</p>
<p>我们来看看 <code v-pre>Closure7.java</code> 和 <code v-pre>Closure8.java</code> 之间的区别。我们看到：在 <code v-pre>Closure7.java</code> 中变量 <code v-pre>i</code> 有过重新赋值。 也许这就是触发<strong>等同 final 效果</strong>错误消息的原因。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Closure9.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Closure9</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上例，重新赋值引用会触发错误消息。如果只修改指向的对象则没问题，只要没有其他人获得对该对象的引用（这意味着你有多个实体可以修改对象，此时事情会变得非常混乱），基本上就是安全的[^6]。</p>
<p>让我们回顾一下 <code v-pre>Closure1.java</code>。那么现在问题来了：为什么变量 <code v-pre>i</code> 被修改编译器却没有报错呢。 它既不是 <code v-pre>final</code> 的，也不是<strong>等同 final 效果</strong>的。因为 <code v-pre>i</code> 是外部类的成员，所以这样做肯定是安全的（除非你正在创建共享可变内存的多个函数）。是的，你可以辩称在这种情况下不会发生变量捕获（Variable Capture）。但可以肯定的是，<code v-pre>Closure3.java</code> 的错误消息是专门针对局部变量的。因此，规则并非只是 “在 Lambda 之外定义的任何变量必须是 <code v-pre>final</code> 的或<strong>等同 final 效果</strong>” 那么简单。相反，你必须考虑捕获的变量是否是<strong>等同 final 效果</strong>的。 如果它是对象中的字段（实例变量），那么它有独立的生命周期，不需要任何特殊的捕获以便稍后在调用 Lambda 时存在。（注：结论是——Lambda 可以没有限制地引用 实例变量和静态变量。但 局部变量必须显式声明为final，或事实上是final 。）</p>
<!-- Inner Classes as Closures -->
<h3 id="作为闭包的内部类" tabindex="-1"><a class="header-anchor" href="#作为闭包的内部类" aria-hidden="true">#</a> 作为闭包的内部类</h3>
<p>我们可以使用匿名内部类重写之前的例子:</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/AnonymousClosure.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">AnonymousClosure</span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>实际上只要有内部类，就会有闭包（Java 8 只是简化了闭包操作）。在 Java 8 之前，变量 <code v-pre>x</code> 和 <code v-pre>i</code> 必须被明确声明为 <code v-pre>final</code>。在 Java 8 中，内部类的规则放宽，包括<strong>等同 final 效果</strong>。</p>
<!-- Function Composition -->
<h2 id="函数组合" tabindex="-1"><a class="header-anchor" href="#函数组合" aria-hidden="true">#</a> 函数组合</h2>
<p>函数组合（Function Composition）意为“多个函数组合成新函数”。它通常是函数式编程的基本组成部分。在前面的 <code v-pre>TransformFunction.java</code> 类中，就有一个使用 <code v-pre>andThen()</code> 的函数组合示例。一些 <code v-pre>java.util.function</code> 接口中包含支持函数组合的方法 [^7]。</p>
<table>
<thead>
<tr>
<th style="text-align:left">组合方法</th>
<th style="text-align:left">支持接口</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left"><code v-pre>andThen(argument)</code> <br> 执行原操作,再执行参数操作</td>
<td style="text-align:left"><strong>Function <br> BiFunction <br> Consumer <br> BiConsumer <br> IntConsumer <br> LongConsumer <br> DoubleConsumer <br> UnaryOperator <br> IntUnaryOperator <br> LongUnaryOperator <br> DoubleUnaryOperator <br> BinaryOperator</strong></td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>compose(argument)</code> <br> 执行参数操作,再执行原操作</td>
<td style="text-align:left"><strong>Function <br> UnaryOperator <br> IntUnaryOperator <br> LongUnaryOperator <br> DoubleUnaryOperator</strong></td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>and(argument)</code>  <br> 原谓词(Predicate)和参数谓词的短路<strong>逻辑与</strong></td>
<td style="text-align:left"><strong>Predicate <br> BiPredicate <br> IntPredicate <br> LongPredicate <br> DoublePredicate</strong></td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>or(argument)</code> <br> 原谓词和参数谓词的短路<strong>逻辑或</strong></td>
<td style="text-align:left"><strong>Predicate <br> BiPredicate <br> IntPredicate <br> LongPredicate <br> DoublePredicate</strong></td>
</tr>
<tr>
<td style="text-align:left"><code v-pre>negate()</code> <br> 该谓词的<strong>逻辑非</strong></td>
<td style="text-align:left"><strong>Predicate <br> BiPredicate <br> IntPredicate <br> LongPredicate <br> DoublePredicate</strong></td>
</tr>
</tbody>
</table>
<p>下例使用了 <code v-pre>Function</code> 里的 <code v-pre>compose()</code>和 <code v-pre>andThen()</code>。代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/FunctionComposition.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>AFTER ALL AMBULANCES
_fter _ll _mbul_nces
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>这里我们重点看正在创建的新函数 <code v-pre>f4</code>。它调用 <code v-pre>apply()</code> 的方式与常规几乎无异[^8]。</p>
<p>当 <code v-pre>f1</code> 获得字符串时，它已经被<code v-pre>f2</code> 剥离了前三个字符。这是因为 <code v-pre>compose（f2）</code> 表示 <code v-pre>f2</code> 的调用发生在 <code v-pre>f1</code> 之前。</p>
<p>下例是 谓词(<code v-pre>Predicate</code>) 的逻辑运算演示.代码示例：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/PredicateComposition.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>foobar
foobaz
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>p4</code> 获取到了所有谓词(<code v-pre>Predicate</code>)并组合成一个更复杂的谓词。解读：如果字符串中不包含 <code v-pre>bar</code> 且长度小于 5，或者它包含 <code v-pre>foo</code> ，则结果为 <code v-pre>true</code>。</p>
<p>正因它产生如此清晰的语法，我在主方法中采用了一些小技巧，并借用了下一章的内容。首先，我创建了一个字符串对象的流，然后将每个对象传递给 <code v-pre>filter()</code> 操作。 <code v-pre>filter()</code> 使用 <code v-pre>p4</code> 的谓词来确定对象的去留。最后我们使用 <code v-pre>forEach()</code> 将 <code v-pre>println</code> 方法引用应用在每个留存的对象上。</p>
<p>从输出结果我们可以看到 <code v-pre>p4</code> 的工作流程：任何带有 <code v-pre>&quot;foo&quot;</code> 的字符串都得以保留，即使它的长度大于 5。 <code v-pre>&quot;fongopuckey&quot;</code> 因长度超出且不包含 <code v-pre>foo</code> 而被丢弃。</p>
<!-- Currying and  Partial Evaluation -->
<h2 id="柯里化和部分求值" tabindex="-1"><a class="header-anchor" href="#柯里化和部分求值" aria-hidden="true">#</a> 柯里化和部分求值</h2>
<p><a href="https://en.wikipedia.org/wiki/Currying" target="_blank" rel="noopener noreferrer">柯里化<ExternalLinkIcon/></a>（Currying）的名称来自于其发明者之一 <em>Haskell Curry</em>。他可能是计算机领域唯一姓氏和名字都命名过重要概念的人（另外就是 Haskell 编程语言）。 柯里化意为：将一个多参数的函数，转换为一系列单参数函数。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/CurryingAndPartials.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Hi Ho
Hi Ho
Hup Ho
Hup Hey
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>[1]</strong> 这一连串的箭头很巧妙。<em>注意</em>，在函数接口声明中，第二个参数是另一个函数。</p>
<p><strong>[2]</strong> 柯里化的目的是能够通过提供单个参数来创建一个新函数，所以现在有了一个“带参函数”和剩下的 “自由函数”（free argument） 。实际上，你从一个双参数函数开始，最后得到一个单参数函数。</p>
<p>我们可以通过继续添加层级来柯里化一个三参数函数：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/Curry3Args.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Hi Ho Hup
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>对于每一级的箭头级联（Arrow-cascading），你都会在类型声明周围包裹另一个 <strong>Function</strong> 。</p>
<p>处理基本类型和装箱时，请使用适当的函数式接口：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// functional/CurriedIntAdd.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>9
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>可以在互联网上找到更多的柯里化示例。通常它们是用 Java 之外的语言实现的，但如果理解了柯里化的基本概念，你可以很轻松地用 Java 实现它们。</p>
<!-- Pure Functional Programming -->
<h2 id="纯函数式编程" tabindex="-1"><a class="header-anchor" href="#纯函数式编程" aria-hidden="true">#</a> 纯函数式编程</h2>
<p>只要多加练习，用没有函数式支持的语言也可以写出纯函数式程序（即使是 C 这样的原始语言）。Java 8 让函数式编程更简单，不过我们要确保一切是 <code v-pre>final</code> 的，同时你的所有方法和函数没有副作用。因为 Java 在本质上并非是不可变语言，所以编译器对我们犯的错误将无能为力。</p>
<p>这种情况下，我们可以借助第三方工具[^9]，但使用 Scala 或 Clojure 这样的语言可能更简单。因为它们从一开始就是为保持不变性而设计的。你可以采用这些语言来编写你的 Java 项目的一部分。如果必须要用纯函数式编写，则可以用 Scala（需要一些练习） 或 Clojure （仅需更少的练习）。虽然 Java 支持<RouterLink to="/OnJava8/24-Concurrent-Programming.html">并发编程</RouterLink>，但如果这是你项目的核心部分，你应该考虑在项目部分功能中使用 <code v-pre>Scala</code> 或 <code v-pre>Clojure</code> 之类的语言。</p>
<!-- Summary -->
<h2 id="本章小结" tabindex="-1"><a class="header-anchor" href="#本章小结" aria-hidden="true">#</a> 本章小结</h2>
<p>Lambda 表达式和方法引用并没有将 Java 转换成函数式语言，而是提供了对函数式编程的支持。这对 Java 来说是一个巨大的改进。因为这允许你编写更简洁明了，易于理解的代码。在下一章中，你会看到它们在 <em>流式编程(streams)</em> 中的应用。相信你会像我一样，喜欢上流式编程。</p>
<p>这些特性满足了很大一部分的、羡慕Clojure 和 Scala 这类更函数化语言的Java程序员。阻止了他们投奔向那些语言（或者至少让他们在投奔之前做好准备）。</p>
...
<!--下面是脚注-->
<p>[^2]: 例如,这个电子书是利用 <a href="http://pandoc.org/" target="_blank" rel="noopener noreferrer">Pandoc<ExternalLinkIcon/></a> 制作出来的，它是用纯函数式语言 <a href="https://www.haskell.org/" target="_blank" rel="noopener noreferrer">Haskell<ExternalLinkIcon/></a> 编写的一个程序 。
[^3]: 有时函数式语言将其描述为“代码即数据”。
[^4]: 这个语法来自 C++。
[^5]: 我还没有验证过这种说法。
[^6]: 当你理解了<RouterLink to="/OnJava8/24-Concurrent-Programming.html">并发编程</RouterLink>章节的内容，你就能明白为什么更改共享变量 “不是线程安全的” 的了。
[^7]: 接口能够支持方法的原因是它们是 Java 8 默认方法，你将在下一章中了解到。
[^8]: 一些语言，如 Python，允许像调用其他函数一样调用组合函数。但这是 Java，所以我们要量力而行。
[^9]: 例如，<a href="https://immutables.github.io/" target="_blank" rel="noopener noreferrer">Immutables<ExternalLinkIcon/></a> 和 <a href="https://mutabilitydetector.github.io/MutabilityDetector/" target="_blank" rel="noopener noreferrer">Mutability Detector<ExternalLinkIcon/></a>。</p>
<!-- 分页 -->
