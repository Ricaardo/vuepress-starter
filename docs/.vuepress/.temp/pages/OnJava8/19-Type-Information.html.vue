...
<!-- Type Information -->
<h1 id="第十九章-类型信息" tabindex="-1"><a class="header-anchor" href="#第十九章-类型信息" aria-hidden="true">#</a> 第十九章 类型信息</h1>
<blockquote>
...
<!-- The Need for RTTI -->
<h2 id="为什么需要-rtti" tabindex="-1"><a class="header-anchor" href="#为什么需要-rtti" aria-hidden="true">#</a> 为什么需要 RTTI</h2>
<p>下面看一下我们已经很熟悉的一个例子，它使用了多态的类层次结构。基类 <code v-pre>Shape</code> 是泛化的类型，从它派生出了三个具体类： <code v-pre>Circle</code> 、<code v-pre>Square</code> 和 <code v-pre>Triangle</code>（见下图所示）。</p>
<p><img src="images/image-20190409114913825-4781754.png" alt="多态例子Shape的类层次结构图"></p>
<p>这是一个典型的类层次结构图，基类位于顶部，派生类向下扩展。面向对象编程的一个基本目的是：让代码只操纵对基类(这里即 <code v-pre>Shape</code> )的引用。这样，如果你想添加一个新类(比如从 <code v-pre>Shape</code> 派生出 <code v-pre>Rhomboid</code>)来扩展程序，就不会影响原来的代码。在这个例子中，<code v-pre>Shape</code> 接口中动态绑定了 <code v-pre>draw()</code> 方法，这样做的目的就是让客户端程序员可以使用泛化的 <code v-pre>Shape</code> 引用来调用 <code v-pre>draw()</code>。<code v-pre>draw()</code> 方法在所有派生类里都会被覆盖，而且由于它是动态绑定的，所以即使通过 <code v-pre>Shape</code> 引用来调用它，也能产生恰当的行为，这就是多态。</p>
<p>因此，我们通常会创建一个具体的对象(<code v-pre>Circle</code>、<code v-pre>Square</code> 或者 <code v-pre>Triangle</code>)，把它向上转型成 <code v-pre>Shape</code> (忽略对象的具体类型)，并且在后面的程序中使用 <code v-pre>Shape</code> 引用来调用在具体对象中被重载的方法（如 <code v-pre>draw()</code>）。</p>
<p>代码如下：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Shapes.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Circle.draw()
Square.draw()
Triangle.draw()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>基类中包含 <code v-pre>draw()</code> 方法，它通过传递 <code v-pre>this</code> 参数传递给 <code v-pre>System.out.println()</code>，间接地使用 <code v-pre>toString()</code> 打印类标识符(注意：这里将 <code v-pre>toString()</code> 声明为 <code v-pre>abstract</code>，以此强制继承者覆盖该方法，并防止对 <code v-pre>Shape</code> 的实例化)。如果某个对象出现在字符串表达式中(涉及&quot;+&quot;和字符串对象的表达式)，<code v-pre>toString()</code> 方法就会被自动调用，以生成表示该对象的 <code v-pre>String</code>。每个派生类都要覆盖（从 <code v-pre>Object</code> 继承来的）<code v-pre>toString()</code> 方法，这样 <code v-pre>draw()</code> 在不同情况下就打印出不同的消息(多态)。</p>
<p>这个例子中，在把 <code v-pre>Shape</code> 对象放入 <code v-pre>Stream&lt;Shape&gt;</code> 中时就会进行向上转型(隐式)，但在向上转型的时候也丢失了这些对象的具体类型。对 <code v-pre>stream</code> 而言，它们只是 <code v-pre>Shape</code> 对象。</p>
<p>严格来说，<code v-pre>Stream&lt;Shape&gt;</code> 实际上是把放入其中的所有对象都当做 <code v-pre>Object</code> 对象来持有，只是取元素时会自动将其类型转为 <code v-pre>Shape</code>。这也是 RTTI 最基本的使用形式，因为在 Java 中，所有类型转换的正确性检查都是在运行时进行的。这也正是 RTTI 的含义所在：在运行时，识别一个对象的类型。</p>
<p>另外在这个例子中，类型转换并不彻底：<code v-pre>Object</code> 被转型为 <code v-pre>Shape</code> ，而不是 <code v-pre>Circle</code>、<code v-pre>Square</code> 或者 <code v-pre>Triangle</code>。这是因为目前我们只能确保这个 <code v-pre>Stream&lt;Shape&gt;</code> 保存的都是 <code v-pre>Shape</code>：</p>
<ul>
<li>编译期，<code v-pre>stream</code> 和 Java 泛型系统确保放入 <code v-pre>stream</code> 的都是 <code v-pre>Shape</code> 对象（<code v-pre>Shape</code> 子类的对象也可视为 <code v-pre>Shape</code> 的对象），否则编译器会报错；</li>
<li>运行时，自动类型转换确保了从 <code v-pre>stream</code> 中取出的对象都是 <code v-pre>Shape</code> 类型。</li>
</ul>
<p>接下来就是多态机制的事了，<code v-pre>Shape</code> 对象实际执行什么样的代码，是由引用所指向的具体对象（<code v-pre>Circle</code>、<code v-pre>Square</code> 或者 <code v-pre>Triangle</code>）决定的。这也符合我们编写代码的一般需求，通常，我们希望大部分代码尽可能少了解对象的具体类型，而是只与对象家族中的一个通用表示打交道（本例中即为 <code v-pre>Shape</code>）。这样，代码会更容易写，更易读和维护；设计也更容易实现，更易于理解和修改。所以多态是面向对象的基本目标。</p>
<p>但是，有时你会碰到一些编程问题，在这些问题中如果你能知道某个泛化引用的具体类型，就可以把问题轻松解决。例如，假设我们允许用户将某些几何形状高亮显示，现在希望找到屏幕上所有高亮显示的三角形；或者，我们现在需要旋转所有图形，但是想跳过圆形(因为圆形旋转没有意义)。这时我们就希望知道 <code v-pre>Stream&lt;Shape&gt;</code> 里边的形状具体是什么类型，而 Java 实际上也满足了我们的这种需求。使用 RTTI，我们可以查询某个 <code v-pre>Shape</code> 引用所指向对象的确切类型，然后选择或者剔除特例。</p>
<!-- The Class Object -->
<h2 id="class-对象" tabindex="-1"><a class="header-anchor" href="#class-对象" aria-hidden="true">#</a> Class 对象</h2>
<p>要理解 RTTI 在 Java 中的工作原理，首先必须知道类型信息在运行时是如何表示的。这项工作是由称为 <strong><code v-pre>Class</code>对象</strong> 的特殊对象完成的，它包含了与类有关的信息。实际上，<code v-pre>Class</code> 对象就是用来创建该类所有&quot;常规&quot;对象的。Java 使用 <code v-pre>Class</code> 对象来实现 RTTI，即便是类型转换这样的操作都是用 <code v-pre>Class</code> 对象实现的。不仅如此，<code v-pre>Class</code> 类还提供了很多使用 RTTI 的其它方式。</p>
<p>类是程序的一部分，每个类都有一个 <code v-pre>Class</code> 对象。换言之，每当我们编写并且编译了一个新类，就会产生一个 <code v-pre>Class</code> 对象（更恰当的说，是被保存在一个同名的 <code v-pre>.class</code> 文件中）。为了生成这个类的对象，Java 虚拟机 (JVM) 先会调用&quot;类加载器&quot;子系统把这个类加载到内存中。</p>
...
<blockquote>
<p>其实构造器也是类的静态方法，虽然构造器前面并没有 <code v-pre>static</code> 关键字。所以，使用 <code v-pre>new</code> 操作符创建类的新对象，这个操作也算作对类的静态成员引用。</p>
</blockquote>
<p>因此，Java 程序在它开始运行之前并没有被完全加载，很多部分是在需要时才会加载。这一点与许多传统编程语言不同，动态加载使得 Java 具有一些静态加载语言（如 C++）很难或者根本不可能实现的特性。</p>
<p>类加载器首先会检查这个类的 <code v-pre>Class</code> 对象是否已经加载，如果尚未加载，默认的类加载器就会根据类名查找 <code v-pre>.class</code> 文件（如果有附加的类加载器，这时候可能就会在数据库中或者通过其它方式获得字节码）。这个类的字节码被加载后，JVM 会对其进行验证，确保它没有损坏，并且不包含不良的 Java 代码(这是 Java 安全防范的一种措施)。</p>
<p>一旦某个类的 <code v-pre>Class</code> 对象被载入内存，它就可以用来创建这个类的所有对象。下面的示范程序可以证明这点：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/SweetShop.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>inside main
Loading Candy
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上面的代码中，<code v-pre>Candy</code>、<code v-pre>Gum</code> 和 <code v-pre>Cookie</code> 这几个类都有一个 <code v-pre>static{...}</code> 静态初始化块，这些静态初始化块在类第一次被加载的时候就会执行。也就是说，静态初始化块会打印出相应的信息，告诉我们这些类分别是什么时候被加载了。而在主方法里边，创建对象的代码都放在了 <code v-pre>print()</code> 语句之间，以帮助我们判断类加载的时间点。</p>
<p>从输出中可以看到，<code v-pre>Class</code> 对象仅在需要的时候才会被加载，<code v-pre>static</code> 初始化是在类加载时进行的。</p>
<p>代码里面还有特别有趣的一行：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token class-name">Class</span><span class="token punctuation">.</span><span class="token function">forName</span><span class="token punctuation">(</span><span class="token string">"Gum"</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>所有 <code v-pre>Class</code> 对象都属于 <code v-pre>Class</code> 类，而且它跟其他普通对象一样，我们可以获取和操控它的引用(这也是类加载器的工作)。<code v-pre>forName()</code> 是 <code v-pre>Class</code> 类的一个静态方法，我们可以使用 <code v-pre>forName()</code> 根据目标类的类名（<code v-pre>String</code>）得到该类的 <code v-pre>Class</code> 对象。上面的代码忽略了 <code v-pre>forName()</code> 的返回值，因为那个调用是为了得到它产生的“副作用”。从结果可以看出，<code v-pre>forName()</code> 执行的副作用是如果 <code v-pre>Gum</code> 类没有被加载就加载它，而在加载的过程中，<code v-pre>Gum</code> 的 <code v-pre>static</code> 初始化块被执行了。</p>
<p>还需要注意的是，如果 <code v-pre>Class.forName()</code> 找不到要加载的类，它就会抛出异常 <code v-pre>ClassNotFoundException</code>。上面的例子中我们只是简单地报告了问题，但在更严密的程序里，就要考虑在异常处理程序中把问题解决掉（具体例子详见<a href="./25-Patterns">设计模式</a>章节）。</p>
<p>无论何时，只要你想在运行时使用类型信息，就必须先得到那个 <code v-pre>Class</code> 对象的引用。<code v-pre>Class.forName()</code> 就是实现这个功能的一个便捷途径，因为使用该方法你不需要先持有这个类型 的对象。但是，如果你已经拥有了目标类的对象，那就可以通过调用 <code v-pre>getClass()</code> 方法来获取 <code v-pre>Class</code> 引用了，这个方法来自根类 <code v-pre>Object</code>，它将返回表示该对象实际类型的 <code v-pre>Class</code> 对象的引用。<code v-pre>Class</code> 包含很多有用的方法，下面代码展示了其中的一部分：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/toys/ToyTest.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Class name: typeinfo.toys.FancyToy is interface?
[false]
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>FancyToy</code> 继承自 <code v-pre>Toy</code> 并实现了 <code v-pre>HasBatteries</code>、<code v-pre>Waterproof</code> 和 <code v-pre>Shoots</code> 接口。在 <code v-pre>main</code> 方法中，我们创建了一个 <code v-pre>Class</code> 引用，然后在 <code v-pre>try</code> 语句里边用 <code v-pre>forName()</code> 方法创建了一个 <code v-pre>FancyToy</code> 的类对象并赋值给该引用。需要注意的是，传递给 <code v-pre>forName()</code> 的字符串必须使用类的全限定名（包含包名）。</p>
<p><code v-pre>printInfo()</code> 函数使用 <code v-pre>getName()</code> 来产生完整类名，使用 <code v-pre>getSimpleName()</code> 产生不带包名的类名，<code v-pre>getCanonicalName()</code> 也是产生完整类名（除内部类和数组外，对大部分类产生的结果与 <code v-pre>getName()</code> 相同）。<code v-pre>isInterface()</code> 用于判断某个 <code v-pre>Class</code> 对象代表的是否为一个接口。因此，通过 <code v-pre>Class</code> 对象，你可以得到关于该类型的所有信息。</p>
<p>在主方法中调用的 <code v-pre>Class.getInterfaces()</code> 方法返回的是存放 <code v-pre>Class</code> 对象的数组，里面的 <code v-pre>Class</code> 对象表示的是那个类实现的接口。</p>
<p>另外，你还可以调用 <code v-pre>getSuperclass()</code> 方法来得到父类的 <code v-pre>Class</code> 对象，再用父类的 <code v-pre>Class</code> 对象调用该方法，重复多次，你就可以得到一个对象完整的类继承结构。</p>
<p><code v-pre>Class</code> 对象的 <code v-pre>newInstance()</code> 方法是实现“虚拟构造器”的一种途径，虚拟构造器可以让你在不知道一个类的确切类型的时候，创建这个类的对象。在前面的例子中，<code v-pre>up</code> 只是一个 <code v-pre>Class</code> 对象的引用，在编译期并不知道这个引用会指向哪个类的 <code v-pre>Class</code> 对象。当你创建新实例时，会得到一个 <code v-pre>Object</code> 引用，但是这个引用指向的是 <code v-pre>Toy</code> 对象。当然，由于得到的是 <code v-pre>Object</code> 引用，目前你只能给它发送 <code v-pre>Object</code> 对象能够接受的调用。而如果你想请求具体对象才有的调用，你就得先获取该对象更多的类型信息，并执行某种转型。另外，使用 <code v-pre>newInstance()</code> 来创建的类，必须带有无参数的构造器。在本章稍后部分，你将会看到如何通过 Java 的反射 API，用任意的构造器来动态地创建类的对象。</p>
<h3 id="类字面常量" tabindex="-1"><a class="header-anchor" href="#类字面常量" aria-hidden="true">#</a> 类字面常量</h3>
<p>Java 还提供了另一种方法来生成类对象的引用：<strong>类字面常量</strong>。对上述程序来说，就像这样：<code v-pre>FancyToy.class;</code>。这样做不仅更简单，而且更安全，因为它在编译时就会受到检查（因此不必放在 <code v-pre>try</code> 语句块中）。并且它根除了对 <code v-pre>forName()</code> 方法的调用，所以效率更高。</p>
<p>类字面常量不仅可以应用于普通类，也可以应用于接口、数组以及基本数据类型。另外，对于基本数据类型的包装类，还有一个标准字段 <code v-pre>TYPE</code>。<code v-pre>TYPE</code> 字段是一个引用，指向对应的基本数据类型的 <code v-pre>Class</code> 对象，如下所示：</p>
<figure>
...
<p>我的建议是使用 <code v-pre>.class</code> 的形式，以保持与普通类的一致性。</p>
<p>注意，有一点很有趣：当使用 <code v-pre>.class</code> 来创建对 <code v-pre>Class</code> 对象的引用时，不会自动地初始化该 <code v-pre>Class</code> 对象。为了使用类而做的准备工作实际包含三个步骤：</p>
<ol>
<li>
<p><strong>加载</strong>，这是由类加载器执行的。该步骤将查找字节码（通常在 classpath 所指定的路径中查找，但这并非是必须的），并从这些字节码中创建一个 <code v-pre>Class</code> 对象。</p>
</li>
<li>
<p><strong>链接</strong>。在链接阶段将验证类中的字节码，为 <code v-pre>static</code> 字段分配存储空间，并且如果需要的话，将解析这个类创建的对其他类的所有引用。</p>
</li>
<li>
<p><strong>初始化</strong>。如果该类具有超类，则先初始化超类，执行 <code v-pre>static</code> 初始化器和 <code v-pre>static</code> 初始化块。</p>
</li>
</ol>
<p>直到第一次引用一个 <code v-pre>static</code> 方法（构造器隐式地是 <code v-pre>static</code>）或者非常量的 <code v-pre>static</code> 字段，才会进行类初始化。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/ClassInitialization.java</span>
...
<span class="token keyword">class</span> <span class="token class-name">Initable</span> <span class="token punctuation">{</span>
    <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token keyword">int</span> <span class="token constant">STATIC_FINAL</span> <span class="token operator">=</span> <span class="token number">47</span><span class="token punctuation">;</span>
    <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token keyword">int</span> <span class="token constant">STATIC_FINAL2</span> <span class="token operator">=</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ClassInitialization</span> <span class="token punctuation">{</span>
    ...
    <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token keyword">throws</span> <span class="token class-name">Exception</span> <span class="token punctuation">{</span>
        ...
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token class-name">Initable</span><span class="token punctuation">.</span><span class="token constant">STATIC_FINAL</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token comment">// Does trigger initialization:</span>
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token class-name">Initable</span><span class="token punctuation">.</span><span class="token constant">STATIC_FINAL2</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token comment">// Does trigger initialization:</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>After creating Initable ref
47
...
74
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>初始化有效地实现了尽可能的“惰性”，从对 <code v-pre>initable</code> 引用的创建中可以看到，仅使用 <code v-pre>.class</code> 语法来获得对类对象的引用不会引发初始化。但与此相反，使用 <code v-pre>Class.forName()</code> 来产生 <code v-pre>Class</code> 引用会立即就进行初始化，如 <code v-pre>initable3</code>。</p>
<p>如果一个 <code v-pre>static final</code> 值是“编译期常量”（如 <code v-pre>Initable.staticFinal</code>），那么这个值不需要对 <code v-pre>Initable</code> 类进行初始化就可以被读取。但是，如果只是将一个字段设置成为 <code v-pre>static</code> 和 <code v-pre>final</code>，还不足以确保这种行为。例如，对 <code v-pre>Initable.staticFinal2</code> 的访问将强制进行类的初始化，因为它不是一个编译期常量。</p>
<p>如果一个 <code v-pre>static</code> 字段不是 <code v-pre>final</code> 的，那么在对它访问时，总是要求在它被读取之前，要先进行链接（为这个字段分配存储空间）和初始化（初始化该存储空间），就像在对 <code v-pre>Initable2.staticNonFinal</code> 的访问中所看到的那样。</p>
<h3 id="泛化的-class-引用" tabindex="-1"><a class="header-anchor" href="#泛化的-class-引用" aria-hidden="true">#</a> 泛化的 <code v-pre>Class</code> 引用</h3>
<p><code v-pre>Class</code> 引用总是指向某个 <code v-pre>Class</code> 对象，而 <code v-pre>Class</code> 对象可以用于产生类的实例，并且包含可作用于这些实例的所有方法代码。它还包含该类的 <code v-pre>static</code> 成员，因此 <code v-pre>Class</code> 引用表明了它所指向对象的确切类型，而该对象便是 <code v-pre>Class</code> 类的一个对象。</p>
<!-- > 译者的理解： `Class` 对象是 `Class` 类产生的对象，而再往深一点说，`Class` 类的 `Class` 对象（`Class.class`）也是其本类产生的对象。即一切皆对象，类也是一种对象。 -->
<p>但是，Java 设计者看准机会，将它的类型变得更具体了一些。Java 引入泛型语法之后，我们可以使用泛型对 <code v-pre>Class</code> 引用所指向的 <code v-pre>Class</code> 对象的类型进行限定。在下面的实例中，两种语法都是正确的：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/GenericClassReferences.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">GenericClassReferences</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>普通的类引用不会产生警告信息。你可以看到，普通的类引用可以重新赋值指向任何其他的 <code v-pre>Class</code> 对象，但是使用泛型限定的类引用只能指向其声明的类型。通过使用泛型语法，我们可以让编译器强制执行额外的类型检查。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>这看起来似乎是起作用的，因为 <code v-pre>Integer</code> 继承自 <code v-pre>Number</code>。但事实却是不行，因为 <code v-pre>Integer</code> 的 <code v-pre>Class</code> 对象并不是 <code v-pre>Number</code>的 <code v-pre>Class</code> 对象的子类（这看起来可能有点诡异，我们将在<a href="./20-Generics">泛型</a>这一章详细讨论）。</p>
<p>为了在使用 <code v-pre>Class</code> 引用时放松限制，我们使用了通配符，它是 Java 泛型中的一部分。通配符就是 <code v-pre>?</code>，表示“任何事物”。因此，我们可以在上例的普通 <code v-pre>Class</code> 引用中添加通配符，并产生相同的结果：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/WildcardClassReferences.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">WildcardClassReferences</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>使用 <code v-pre>Class&lt;?&gt;</code> 比单纯使用 <code v-pre>Class</code> 要好，虽然它们是等价的，并且单纯使用 <code v-pre>Class</code> 不会产生编译器警告信息。使用 <code v-pre>Class&lt;?&gt;</code> 的好处是它表示你并非是碰巧或者由于疏忽才使用了一个非具体的类引用，而是特意为之。</p>
<p>为了创建一个限定指向某种类型或其子类的 <code v-pre>Class</code> 引用，我们需要将通配符与 <code v-pre>extends</code> 关键字配合使用，创建一个范围限定。这与仅仅声明 <code v-pre>Class&lt;Number&gt;</code> 不同，现在做如下声明：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/BoundedClassReferences.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">BoundedClassReferences</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>向 <code v-pre>Class</code> 引用添加泛型语法的原因只是为了提供编译期类型检查，因此如果你操作有误，稍后就会发现这点。使用普通的 <code v-pre>Class</code> 引用你要确保自己不会犯错，因为一旦你犯了错误，就要等到运行时才能发现它，很不方便。</p>
<p>下面的示例使用了泛型语法，它保存了一个类引用，稍后又用 <code v-pre>newInstance()</code> 方法产生类的对象：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/DynamicSupplier.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>10
11
12
13
14
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注意，这个类必须假设与它一起工作的任何类型都有一个无参构造器，否则运行时会抛出异常。编译期对该程序不会产生任何警告信息。</p>
<p>当你将泛型语法用于 <code v-pre>Class</code> 对象时，<code v-pre>newInstance()</code> 将返回该对象的确切类型，而不仅仅只是在 <code v-pre>ToyTest.java</code> 中看到的基类 <code v-pre>Object</code>。然而，这在某种程度上有些受限：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/toys/GenericToyTest.java</span>
<span class="token comment">// 测试 Class 类</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">GenericToyTest</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果你手头的是超类，那编译器将只允许你声明超类引用为“某个类，它是 <code v-pre>FancyToy</code> 的超类”，就像在表达式 <code v-pre>Class&lt;? super FancyToy&gt;</code> 中所看到的那样。而不会接收 <code v-pre>Class&lt;Toy&gt;</code> 这样的声明。这看上去显得有些怪，因为 <code v-pre>getSuperClass()</code> 方法返回的是基类（不是接口），并且编译器在编译期就知道它是什么类型了（在本例中就是 <code v-pre>Toy.class</code>），而不仅仅只是&quot;某个类&quot;。不管怎样，正是由于这种含糊性，<code v-pre>up.newInstance</code> 的返回值不是精确类型，而只是 <code v-pre>Object</code>。</p>
<h3 id="cast-方法" tabindex="-1"><a class="header-anchor" href="#cast-方法" aria-hidden="true">#</a> <code v-pre>cast()</code> 方法</h3>
<p>Java 中还有用于 <code v-pre>Class</code> 引用的转型语法，即 <code v-pre>cast()</code> 方法：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/ClassCasts.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ClassCasts</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>cast()</code> 方法接受参数对象，并将其类型转换为 <code v-pre>Class</code> 引用的类型。但是，如果观察上面的代码，你就会发现，与实现了相同功能的 <code v-pre>main</code> 方法中最后一行相比，这种转型好像做了很多额外的工作。</p>
<p><code v-pre>cast()</code> 在无法使用普通类型转换的情况下会显得非常有用，在你编写泛型代码（你将在<a href="./20-Generics">泛型</a>这一章学习到）时，如果你保存了 <code v-pre>Class</code> 引用，并希望以后通过这个引用来执行转型，你就需要用到 <code v-pre>cast()</code>。但事实却是这种情况非常少见，我发现整个 Java 类库中，只有一处使用了 <code v-pre>cast()</code>（在 <code v-pre>com.sun.mirror.util.DeclarationFilter</code> 中）。</p>
<p>Java 类库中另一个没有任何用处的特性就是 <code v-pre>Class.asSubclass()</code>，该方法允许你将一个 <code v-pre>Class</code> 对象转型为更加具体的类型。</p>
<h2 id="类型转换检测" tabindex="-1"><a class="header-anchor" href="#类型转换检测" aria-hidden="true">#</a> 类型转换检测</h2>
<p>直到现在，我们已知的 RTTI 类型包括：</p>
<ol>
<li>
<p>传统的类型转换，如 “<code v-pre>(Shape)</code>”，由 RTTI 确保转换的正确性，如果执行了一个错误的类型转换，就会抛出一个 <code v-pre>ClassCastException</code> 异常。</p>
</li>
<li>
<p>代表对象类型的 <code v-pre>Class</code> 对象. 通过查询 <code v-pre>Class</code> 对象可以获取运行时所需的信息.</p>
</li>
</ol>
<p>在 C++ 中，经典的类型转换 “<code v-pre>(Shape)</code>” 并不使用 RTTI。它只是简单地告诉编译器将这个对象作为新的类型对待. 而 Java 会进行类型检查，这种类型转换一般被称作“类型安全的向下转型”。之所以称作“向下转型”，是因为传统上类继承图是这么画的。将 <code v-pre>Circle</code> 转换为 <code v-pre>Shape</code> 是一次向上转型, 将 <code v-pre>Shape</code> 转换为 <code v-pre>Circle</code> 是一次向下转型。但是, 因为我们知道 <code v-pre>Circle</code> 肯定是一个 <code v-pre>Shape</code>，所以编译器允许我们自由地做向上转型的赋值操作，且不需要任何显式的转型操作。当你给编译器一个 <code v-pre>Shape</code> 的时候，编译器并不知道它到底是什么类型的 <code v-pre>Shape</code>——它可能是 <code v-pre>Shape</code>，也可能是 <code v-pre>Shape</code> 的子类型，例如 <code v-pre>Circle</code>、<code v-pre>Square</code>、<code v-pre>Triangle</code> 或某种其他的类型。在编译期，编译器只能知道它是 <code v-pre>Shape</code>。因此，你需要使用显式地进行类型转换，以告知编译器你想转换的特定类型，否则编译器就不允许你执行向下转型赋值。 （编译器将会检查向下转型是否合理，因此它不允许向下转型到实际不是待转型类型的子类类型上）。</p>
<p>RTTI 在 Java 中还有第三种形式，那就是关键字 <code v-pre>instanceof</code>。它返回一个布尔值，告诉我们对象是不是某个特定类型的实例，可以用提问的方式使用它，就像这个样子：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">if</span><span class="token punctuation">(</span>x <span class="token keyword">instanceof</span> <span class="token class-name">Dog</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>在将 <code v-pre>x</code> 的类型转换为 <code v-pre>Dog</code> 之前，<code v-pre>if</code> 语句会先检查 <code v-pre>x</code> 是否是 <code v-pre>Dog</code> 类型的对象。进行向下转型前，如果没有其他信息可以告诉你这个对象是什么类型，那么使用 <code v-pre>instanceof</code> 是非常重要的，否则会得到一个 <code v-pre>ClassCastException</code> 异常。</p>
<p>一般，可能想要查找某种类型（比如要找三角形，并填充为紫色），这时可以轻松地使用 <code v-pre>instanceof</code> 来度量所有对象。举个例子，假如你有一个类的继承体系，描述了 <code v-pre>Pet</code>（以及它们的主人，在后面一个例子中会用到这个特性）。在这个继承体系中的每个 <code v-pre>Individual</code> 都有一个 <code v-pre>id</code> 和一个可选的名字。尽管下面的类都继承自 <code v-pre>Individual</code>，但是 <code v-pre>Individual</code> 类复杂性较高，因此其代码将放在<a href="./Appendix-Collection-Topics">附录：容器</a>中进行解释说明。正如你所看到的，此处并不需要去了解 <code v-pre>Individual</code> 的代码——你只需了解你可以创建其具名或不具名的对象，并且每个 <code v-pre>Individual</code> 都有一个 <code v-pre>id()</code> 方法，如果你没有为 <code v-pre>Individual</code> 提供名字，<code v-pre>toString()</code> 方法只产生类型名。</p>
<p>下面是继承自 <code v-pre>Individual</code> 的类的继承体系：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/pets/Person.java</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们必须显式地为每一个子类编写无参构造器。因为我们有一个带一个参数的构造器，所以编译器不会自动地为我们加上无参构造器。</p>
<p>接下来，我们需要一个类，它可以随机地创建不同类型的宠物，同时，它还可以创建宠物数组和持有宠物的 <code v-pre>List</code>。为了使这个类更加普遍适用，我们将其定义为抽象类：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/pets/PetCreator.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">abstract</span> <span class="token keyword">class</span> <span class="token class-name">PetCreator</span> <span class="token keyword">implements</span> <span class="token class-name">Supplier</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Pet</span><span class="token punctuation">></span></span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>抽象的 <code v-pre>types()</code> 方法需要子类来实现，以此来获取 <code v-pre>Class</code> 对象构成的 <code v-pre>List</code>（这是模板方法设计模式的一种变体）。注意，其中类的类型被定义为“任何从 <code v-pre>Pet</code> 导出的类型”，因此 <code v-pre>newInstance()</code> 不需要转型就可以产生 <code v-pre>Pet</code>。<code v-pre>get()</code> 随机的选取出一个 <code v-pre>Class</code> 对象，然后可以通过 <code v-pre>Class.newInstance()</code> 来生成该类的新实例。</p>
<p>在调用 <code v-pre>newInstance()</code> 时，可能会出现两种异常。在紧跟 <code v-pre>try</code> 语句块后面的 <code v-pre>catch</code> 子句中可以看到对它们的处理。异常的名字再次成为了一种对错误类型相对比较有用的解释（<code v-pre>IllegalAccessException</code> 违反了 Java 安全机制，在本例中，表示默认构造器为 <code v-pre>private</code> 的情况）。</p>
<p>当你创建 <code v-pre>PetCreator</code> 的子类时，你需要为 <code v-pre>get()</code> 方法提供 <code v-pre>Pet</code> 类型的 <code v-pre>List</code>。<code v-pre>types()</code> 方法会简单地返回一个静态 <code v-pre>List</code> 的引用。下面是使用 <code v-pre>forName()</code> 的一个具体实现：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/pets/ForNameCreator.java</span>
<span class="token keyword">package</span> <span class="token namespace">typeinfo<span class="token punctuation">.</span>pets</span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">java<span class="token punctuation">.</span>util<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">ForNameCreator</span> <span class="token keyword">extends</span> <span class="token class-name">PetCreator</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>loader()</code> 方法使用 <code v-pre>Class.forName()</code> 创建了 <code v-pre>Class</code> 对象的 <code v-pre>List</code>。这可能会导致 <code v-pre>ClassNotFoundException</code> 异常，因为你传入的是一个 <code v-pre>String</code> 类型的参数，它不能在编译期间被确认是否合理。由于 <code v-pre>Pet</code> 相关的文件在 <code v-pre>typeinfo</code> 包里面，所以使用它们的时候需要填写完整的包名。</p>
<p>为了使得 <code v-pre>List</code> 装入的是具体的 <code v-pre>Class</code> 对象，类型转换是必须的，它会产生一个编译时警告。<code v-pre>loader()</code> 方法是分开编写的，然后它被放入到一个静态代码块里，因为 <code v-pre>@SuppressWarning</code> 注解不能够直接放置在静态代码块之上。</p>
<p>为了对 <code v-pre>Pet</code> 进行计数，我们需要一个能跟踪不同类型的 <code v-pre>Pet</code> 的工具。<code v-pre>Map</code> 是这个需求的首选，我们将 <code v-pre>Pet</code> 类型名作为键，将保存 <code v-pre>Pet</code> 数量的 <code v-pre>Integer</code> 作为值。通过这种方式，你就可以询问：“有多少个 <code v-pre>Hamster</code> 对象？”我们可以使用 <code v-pre>instanceof</code> 来对 <code v-pre>Pet</code> 进行计数：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/PetCount.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Rat Manx Cymric Mutt Pug Cymric Pug Manx Cymric Rat
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 <code v-pre>countPets()</code> 中，一个简短的静态方法 <code v-pre>Pets.array()</code> 生产出了一个随机动物的集合。每个 <code v-pre>Pet</code> 都被 <code v-pre>instanceof</code> 检测到并计算了一遍。</p>
<p><code v-pre>instanceof</code> 有一个严格的限制：只可以将它与命名类型进行比较，而不能与 <code v-pre>Class</code> 对象作比较。在前面的例子中，你可能会觉得写出一大堆 <code v-pre>instanceof</code> 表达式很乏味，事实也是如此。但是，也没有办法让 <code v-pre>instanceof</code> 聪明起来，让它能够自动地创建一个 <code v-pre>Class</code> 对象的数组，然后将目标与这个数组中的对象逐一进行比较（稍后会看到一种替代方案）。其实这并不是那么大的限制，如果你在程序中写了大量的 <code v-pre>instanceof</code>，那就说明你的设计可能存在瑕疵。</p>
<h3 id="使用类字面量" tabindex="-1"><a class="header-anchor" href="#使用类字面量" aria-hidden="true">#</a> 使用类字面量</h3>
<p>如果我们使用类字面量重新实现 <code v-pre>PetCreator</code> 类的话，其结果在很多方面都会更清晰：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/pets/LiteralPetCreator.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">LiteralPetCreator</span> <span class="token keyword">extends</span> <span class="token class-name">PetCreator</span> <span class="token punctuation">{</span>
    ...
    <span class="token keyword">private</span> <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token class-name">List</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Class</span><span class="token punctuation">&lt;</span><span class="token operator">?</span> <span class="token keyword">extends</span> <span class="token class-name">Pet</span><span class="token punctuation">></span><span class="token punctuation">></span></span> <span class="token constant">TYPES</span> <span class="token operator">=</span>
            <span class="token constant">ALL_TYPES</span><span class="token punctuation">.</span><span class="token function">subList</span><span class="token punctuation">(</span><span class="token constant">ALL_TYPES</span><span class="token punctuation">.</span><span class="token function">indexOf</span><span class="token punctuation">(</span><span class="token class-name">Mutt</span><span class="token punctuation">.</span><span class="token keyword">class</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
                    <span class="token constant">ALL_TYPES</span><span class="token punctuation">.</span><span class="token function">size</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">)</span><span class="token punctuation">;</span>

    <span class="token annotation punctuation">@Override</span>
    <span class="token keyword">public</span> <span class="token class-name">List</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Class</span><span class="token punctuation">&lt;</span><span class="token operator">?</span> <span class="token keyword">extends</span> <span class="token class-name">Pet</span><span class="token punctuation">></span><span class="token punctuation">></span></span> <span class="token function">types</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">return</span> <span class="token constant">TYPES</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>

    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">System</span><span class="token punctuation">.</span>out<span class="token punctuation">.</span><span class="token function">println</span><span class="token punctuation">(</span><span class="token constant">TYPES</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>输出结果：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>[class typeinfo.pets.Mutt, class typeinfo.pets.Pug,
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在即将到来的 <code v-pre>PetCount3.java</code> 示例中，我们用所有 <code v-pre>Pet</code> 类型预先加载一个 <code v-pre>Map</code>（不仅仅是随机生成的），因此 <code v-pre>ALL_TYPES</code> 类型的列表是必要的。<code v-pre>types</code> 列表是 <code v-pre>ALL_TYPES</code> 类型（使用 <code v-pre>List.subList()</code> 创建）的一部分，它包含精确的宠物类型，因此用于随机生成 <code v-pre>Pet</code>。</p>
<p>这次，<code v-pre>types</code> 的创建没有被 <code v-pre>try</code> 块包围，因为它是在编译时计算的，因此不会引发任何异常，不像 <code v-pre>Class.forName()</code>。</p>
<p>我们现在在 <code v-pre>typeinfo.pets</code> 库中有两个 <code v-pre>PetCreator</code> 的实现。为了提供第二个作为默认实现，我们可以创建一个使用 <code v-pre>LiteralPetCreator</code> 的 <em>外观模式</em>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/pets/Pets.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Pets</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">final</span> <span class="token class-name">PetCreator</span> <span class="token constant">CREATOR</span> <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">LiteralPetCreator</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token class-name">Pet</span> <span class="token function">get</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">return</span> <span class="token constant">CREATOR</span><span class="token punctuation">.</span><span class="token function">get</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
    ...
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token class-name">Pet</span><span class="token punctuation">[</span><span class="token punctuation">]</span> <span class="token function">array</span><span class="token punctuation">(</span><span class="token keyword">int</span> size<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token keyword">for</span> <span class="token punctuation">(</span><span class="token keyword">int</span> i <span class="token operator">=</span> <span class="token number">0</span><span class="token punctuation">;</span> i <span class="token operator">&lt;</span> size<span class="token punctuation">;</span> i<span class="token operator">++</span><span class="token punctuation">)</span>
            result<span class="token punctuation">[</span>i<span class="token punctuation">]</span> <span class="token operator">=</span> <span class="token constant">CREATOR</span><span class="token punctuation">.</span><span class="token function">get</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    ...
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token class-name">Stream</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Pet</span><span class="token punctuation">></span></span> <span class="token function">stream</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">return</span> <span class="token class-name">Stream</span><span class="token punctuation">.</span><span class="token function">generate</span><span class="token punctuation">(</span><span class="token constant">CREATOR</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这还提供了对 <code v-pre>get()</code>、<code v-pre>array()</code> 和 <code v-pre>list()</code> 的间接调用，以及生成 <code v-pre>Stream&lt;Pet&gt;</code> 的新方法。</p>
<p>因为 <code v-pre>PetCount.countPets()</code> 采用了 <code v-pre>PetCreator</code> 参数，所以我们可以很容易地测试 <code v-pre>LiteralPetCreator</code>（通过上面的外观模式）：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/PetCount2.java</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">typeinfo<span class="token punctuation">.</span>pets<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PetCount2</span> <span class="token punctuation">{</span>
    <span class="token keyword">public</span> <span class="token keyword">static</span> <span class="token keyword">void</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token class-name">String</span><span class="token punctuation">[</span><span class="token punctuation">]</span> args<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token class-name">PetCount</span><span class="token punctuation">.</span><span class="token function">countPets</span><span class="token punctuation">(</span><span class="token class-name">Pets</span><span class="token punctuation">.</span><span class="token constant">CREATOR</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
...
{EgyptianMau=2, Pug=3, Rat=2, Cymric=5, Mouse=2, Cat=9,
Manx=7, Rodent=5, Mutt=3, Dog=6, Pet=20, Hamster=1}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>输出与 <code v-pre>PetCount.java</code> 的输出相同。</p>
<h3 id="一个动态-instanceof-函数" tabindex="-1"><a class="header-anchor" href="#一个动态-instanceof-函数" aria-hidden="true">#</a> 一个动态 <code v-pre>instanceof</code> 函数</h3>
<p><code v-pre>Class.isInstance()</code> 方法提供了一种动态测试对象类型的方法。因此，所有这些繁琐的 <code v-pre>instanceof</code> 语句都可以从 <code v-pre>PetCount.java</code> 中删除：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/PetCount3.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">PetCount3</span> <span class="token punctuation">{</span>
    <span class="token keyword">static</span> <span class="token keyword">class</span> <span class="token class-name">Counter</span> <span class="token keyword">extends</span>
        ...
        <span class="token class-name">Counter</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
            <span class="token keyword">super</span><span class="token punctuation">(</span><span class="token class-name">LiteralPetCreator</span><span class="token punctuation">.</span><span class="token constant">ALL_TYPES</span><span class="token punctuation">.</span><span class="token function">stream</span><span class="token punctuation">(</span><span class="token punctuation">)</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>为了计算所有不同类型的 <code v-pre>Pet</code>，<code v-pre>Counter Map</code> 预先加载了来自 <code v-pre>LiteralPetCreator.ALL_TYPES</code> 的类型。如果不预先加载 <code v-pre>Map</code>，将只计数随机生成的类型，而不是像 <code v-pre>Pet</code> 和 <code v-pre>Cat</code> 这样的基本类型。</p>
<p><code v-pre>isInstance()</code> 方法消除了对 <code v-pre>instanceof</code> 表达式的需要。此外，这意味着你可以通过更改 <code v-pre>LiteralPetCreator.types</code> 数组来添加新类型的 <code v-pre>Pet</code>；程序的其余部分不需要修改（就像使用 <code v-pre>instanceof</code> 表达式时那样）。</p>
<p><code v-pre>toString()</code> 方法被重载，以便更容易读取输出，该输出仍与打印 <code v-pre>Map</code> 时看到的典型输出匹配。</p>
<h3 id="递归计数" tabindex="-1"><a class="header-anchor" href="#递归计数" aria-hidden="true">#</a> 递归计数</h3>
<p><code v-pre>PetCount3.Counter</code> 中的 <code v-pre>Map</code> 预先加载了所有不同的 <code v-pre>Pet</code> 类。我们可以使用 <code v-pre>Class.isAssignableFrom()</code> 而不是预加载 <code v-pre>Map</code> ，并创建一个不限于计数 <code v-pre>Pet</code> 的通用工具：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// onjava/TypeCounter.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">TypeCounter</span> <span class="token keyword">extends</span> <span class="token class-name">HashMap</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Class</span><span class="token punctuation">&lt;</span><span class="token operator">?</span><span class="token punctuation">></span><span class="token punctuation">,</span> <span class="token class-name">Integer</span><span class="token punctuation">></span></span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>count()</code> 方法获取其参数的 <code v-pre>Class</code>，并使用 <code v-pre>isAssignableFrom()</code> 进行运行时检查，以验证传递的对象实际上属于感兴趣的层次结构。<code v-pre>countClass()</code> 首先计算类的确切类型。然后，如果 <code v-pre>baseType</code> 可以从超类赋值，则在超类上递归调用 <code v-pre>countClass()</code>。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/PetCount4.java</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">typeinfo<span class="token punctuation">.</span>pets<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
<span class="token keyword">import</span> <span class="token import"><span class="token namespace">onjava<span class="token punctuation">.</span></span><span class="token operator">*</span></span><span class="token punctuation">;</span>
...
Mouse=2}
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>输出表明两个基类型以及精确类型都被计数了。</p>
<!-- Registered Factories -->
<h2 id="注册工厂" tabindex="-1"><a class="header-anchor" href="#注册工厂" aria-hidden="true">#</a> 注册工厂</h2>
<p>从 <code v-pre>Pet</code> 层次结构生成对象的问题是，每当向层次结构中添加一种新类型的 <code v-pre>Pet</code> 时，必须记住将其添加到 <code v-pre>LiteralPetCreator.java</code> 的条目中。在一个定期添加更多类的系统中，这可能会成为问题。</p>
...
<p>我们在这里所做的另一个更改是使用<em>工厂方法</em>设计模式将对象的创建推迟到类本身。工厂方法可以以多态方式调用，并为你创建适当类型的对象。事实证明，<code v-pre>java.util.function.Supplier</code> 用 <code v-pre>T get()</code> 描述了原型工厂方法。协变返回类型允许 <code v-pre>get()</code> 为 <code v-pre>Supplier</code> 的每个子类实现返回不同的类型。</p>
<p>在本例中，基类 <code v-pre>Part</code> 包含一个工厂对象的静态列表，列表成员类型为 <code v-pre>Supplier&lt;Part&gt;</code>。对于应该由 <code v-pre>get()</code> 方法生成的类型的工厂，通过将它们添加到 <code v-pre>prototypes</code> 列表向基类“注册”。奇怪的是，这些工厂本身就是对象的实例。此列表中的每个对象都是用于创建其他对象的<em>原型</em>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/RegisteredFactories.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>GeneratorBelt
CabinAirFilter
GeneratorBelt
AirFilter
PowerSteeringBelt
...
PowerSteeringBelt
FuelFilter
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>并非层次结构中的所有类都应实例化；这里的 <code v-pre>Filter</code> 和 <code v-pre>Belt</code> 只是分类器，这样你就不会创建任何一个类的实例，而是只创建它们的子类（请注意，如果尝试这样做，你将获得 <code v-pre>Part</code> 基类的行为）。</p>
<p>因为 <code v-pre>Part implements Supplier&lt;Part&gt;</code>，<code v-pre>Part</code> 通过其 <code v-pre>get()</code> 方法供应其他 <code v-pre>Part</code>。如果为基类 <code v-pre>Part</code> 调用 <code v-pre>get()</code>（或者如果 <code v-pre>generate()</code> 调用 <code v-pre>get()</code>），它将创建随机特定的 <code v-pre>Part</code> 子类型，每个子类型最终都从 <code v-pre>Part</code> 继承，并重写相应的 <code v-pre>get()</code> 以生成它们中的一个。</p>
<!-- Instanceof vs. Class Equivalence -->
<h2 id="类的等价比较" tabindex="-1"><a class="header-anchor" href="#类的等价比较" aria-hidden="true">#</a> 类的等价比较</h2>
<p>当你查询类型信息时，需要注意：instanceof 的形式(即 <code v-pre>instanceof</code> 或 <code v-pre>isInstance()</code> ，这两者产生的结果相同) 和 与 Class 对象直接比较 这两者间存在重要区别。下面的例子展示了这种区别：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/FamilyVsExactType.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Testing x of type class typeinfo.Base
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>test()</code> 方法使用两种形式的 <code v-pre>instanceof</code> 对其参数执行类型检查。然后，它获取 <code v-pre>Class</code> 引用，并使用 <code v-pre>==</code> 和 <code v-pre>equals()</code> 测试 <code v-pre>Class</code> 对象的相等性。令人放心的是，<code v-pre>instanceof</code> 和 <code v-pre>isInstance()</code> 产生的结果相同， <code v-pre>equals()</code> 和 <code v-pre>==</code> 产生的结果也相同。但测试本身得出了不同的结论。与类型的概念一致，<code v-pre>instanceof</code> 说的是“你是这个类，还是从这个类派生的类？”。而如果使用 <code v-pre>==</code> 比较实际的<code v-pre>Class</code> 对象，则与继承无关 —— 它要么是确切的类型，要么不是。</p>
<!-- Reflection: Runtime Class Information -->
<h2 id="反射-运行时类信息" tabindex="-1"><a class="header-anchor" href="#反射-运行时类信息" aria-hidden="true">#</a> 反射：运行时类信息</h2>
...
<p>类 <code v-pre>Class</code> 支持<em>反射</em>的概念， <code v-pre>java.lang.reflect</code> 库中包含类 <code v-pre>Field</code>、<code v-pre>Method</code> 和 <code v-pre>Constructor</code>（每一个都实现了 <code v-pre>Member</code> 接口）。这些类型的对象由 JVM 在运行时创建，以表示未知类中的对应成员。然后，可以使用 <code v-pre>Constructor</code> 创建新对象，<code v-pre>get()</code> 和 <code v-pre>set()</code> 方法读取和修改与 <code v-pre>Field</code> 对象关联的字段，<code v-pre>invoke()</code> 方法调用与 <code v-pre>Method</code> 对象关联的方法。此外，还可以调用便利方法 <code v-pre>getFields()</code>、<code v-pre>getMethods()</code>、<code v-pre>getConstructors()</code> 等，以返回表示字段、方法和构造函数的对象数组。（你可以通过在 JDK 文档中查找类 <code v-pre>Class</code> 来了解更多信息。）因此，匿名对象的类信息可以在运行时完全确定，编译时不需要知道任何信息。</p>
<p>重要的是要意识到反射没有什么魔力。当你使用反射与未知类型的对象交互时，JVM 将查看该对象，并看到它属于特定的类（就像普通的 RTTI）。在对其执行任何操作之前，必须加载 <code v-pre>Class</code> 对象。因此，该特定类型的 <code v-pre>.class</code> 文件必须在本地计算机上或通过网络对 JVM 仍然可用。因此，RTTI 和反射的真正区别在于，使用 RTTI 时，编译器在编译时会打开并检查 <code v-pre>.class</code> 文件。换句话说，你可以用“正常”的方式调用一个对象的所有方法。通过反射，<code v-pre>.class</code> 文件在编译时不可用；它由运行时环境打开并检查。</p>
<h3 id="类方法提取器" tabindex="-1"><a class="header-anchor" href="#类方法提取器" aria-hidden="true">#</a> 类方法提取器</h3>
<p>通常，你不会直接使用反射工具，但它们可以帮助你创建更多的动态代码。反射是用来支持其他 Java 特性的，例如对象序列化（参见<a href="Appendix-Object-Serialization">附录：对象序列化</a>）。但是，有时动态提取有关类的信息很有用。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/ShowMethods.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>public static void main(String[])
...
public ShowMethods()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Class</code> 方法 <code v-pre>getmethods()</code> 和 <code v-pre>getconstructors()</code>  分别返回 <code v-pre>Method</code> 数组和 <code v-pre>Constructor</code> 数组。这些类中的每一个都有进一步的方法来解析它们所表示的方法的名称、参数和返回值。但你也可以像这里所做的那样，使用 <code v-pre>toString()</code>，生成带有整个方法签名的 <code v-pre>String</code>。代码的其余部分提取命令行信息，确定特定签名是否与目标 <code v-pre>String</code>（使用 <code v-pre>indexOf()</code>）匹配，并使用正则表达式（在 <a href="#ch021.xhtml#strings">Strings</a> 一章中介绍）删除名称限定符。</p>
<p>编译时无法知道 <code v-pre>Class.forName()</code> 生成的结果，因此所有方法签名信息都是在运行时提取的。如果你研究 JDK 反射文档，你将看到有足够的支持来实际设置和对编译时完全未知的对象进行方法调用（本书后面有这样的例子）。虽然最初你可能认为你永远都不需要这样做，但是反射的全部价值可能会令人惊讶。</p>
<p>上面的输出来自命令行：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code>java <span class="token class-name">ShowMethods</span> <span class="token class-name">ShowMethods</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>输出包含一个 <code v-pre>public</code> 无参数构造函数，即使未定义构造函数。你看到的构造函数是由编译器自动合成的。如果将 <code v-pre>ShowMethods</code> 设置为非 <code v-pre>public</code> 类（即只有包级访问权），则合成的无参数构造函数将不再显示在输出中。自动为合成的无参数构造函数授予与类相同的访问权。</p>
<p>尝试运行 <code v-pre>java ShowMethods java.lang.String</code>，并附加一个 <code v-pre>char</code>、<code v-pre>int</code>、<code v-pre>String</code> 等参数。</p>
<p>编程时，当你不记得某个类是否有特定的方法，并且不想在 JDK 文档中搜索索引或类层次结构时，或者如果你不知道该类是否可以对 <code v-pre>Color</code> 对象执行任何操作时，该工具能节省不少时间。</p>
<!-- Dynamic Proxies -->
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/SimpleProxyDemo.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>doSomething
somethingElse bonobo
SimpleProxy doSomething
doSomething
SimpleProxy somethingElse bonobo
somethingElse bonobo
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>因为 <code v-pre>consumer()</code> 接受 <code v-pre>Interface</code>，所以它不知道获得的是 <code v-pre>RealObject</code> 还是 <code v-pre>SimpleProxy</code>，因为两者都实现了 <code v-pre>Interface</code>。
但是，在客户端和 <code v-pre>RealObject</code> 之间插入的 <code v-pre>SimpleProxy</code> 执行操作，然后在 <code v-pre>RealObject</code> 上调用相同的方法。</p>
<p>当你希望将额外的操作与“真实对象”做分离时，代理可能会有所帮助，尤其是当你想要轻松地启用额外的操作时，反之亦然（设计模式就是封装变更---所以你必须改变一些东西以证明模式的合理性）。例如，如果你想跟踪对 <code v-pre>RealObject</code> 中方法的调用，或衡量此类调用的开销，该怎么办？你不想这部分代码耦合到你的程序中，而代理能使你可以很轻松地添加或删除它。</p>
<p>Java 的<em>动态代理</em>更进一步，不仅动态创建代理对象而且动态处理对代理方法的调用。在动态代理上进行的所有调用都被重定向到单个<em>调用处理程序</em>，该处理程序负责发现调用的内容并决定如何处理。这是 <code v-pre>SimpleProxyDemo.java</code> 使用动态代理重写的例子：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/SimpleDynamicProxy.java</span>
...
[Ljava.lang.Object;@6bc7c054
  bonobo
somethingElse bonobo
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>可以通过调用静态方法 <code v-pre>Proxy.newProxyInstance()</code> 来创建动态代理，该方法需要一个类加载器（通常可以从已加载的对象中获取），希望代理实现的接口列表（不是类或抽象类），以及接口  <code v-pre>InvocationHandler</code> 的一个实现。动态代理会将所有调用重定向到调用处理程序，因此通常为调用处理程序的构造函数提供对“真实”对象的引用，以便一旦执行中介任务便可以转发请求。</p>
<p><code v-pre>invoke()</code> 方法被传递给代理对象，以防万一你必须区分请求的来源---但是在很多情况下都无需关心。但是，在 <code v-pre>invoke()</code> 内的代理上调用方法时要小心，因为接口的调用是通过代理重定向的。</p>
<p>通常执行代理操作，然后使用 <code v-pre>Method.invoke()</code> 将请求转发给被代理对象，并携带必要的参数。这在一开始看起来是有限制的，好像你只能执行一般的操作。但是，可以过滤某些方法调用，同时传递其他方法调用：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/SelectingMethods.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>boring1
boring2
Proxy detected the interesting method
interesting bonobo
boring3
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个示例里，我们只是在寻找方法名，但是你也可以寻找方法签名的其他方面，甚至可以搜索特定的参数值。</p>
<p>动态代理不是你每天都会使用的工具，但是它可以很好地解决某些类型的问题。你可以在 Erich Gamma 等人的<em>设计模式</em>中了解有关<em>代理</em>和其他设计模式的更多信息。 （Addison-Wesley，1995年），以及<RouterLink to="/OnJava8/25-Patterns.html">设计模式</RouterLink>一章。</p>
<!-- Using Optional -->
<h2 id="optional类" tabindex="-1"><a class="header-anchor" href="#optional类" aria-hidden="true">#</a> Optional类</h2>
<p>如果你使用内置的 <code v-pre>null</code> 来表示没有对象，每次使用引用的时候就必须测试一下引用是否为 <code v-pre>null</code>，这显得有点枯燥，而且势必会产生相当乏味的代码。问题在于 <code v-pre>null</code> 没什么自己的行为，只会在你想用它执行任何操作的时候产生 <code v-pre>NullPointException</code>。<code v-pre>java.util.Optional</code>（首次出现是在<RouterLink to="/OnJava8/13-Functional-Programming.html">函数式编程</RouterLink>这章）为 <code v-pre>null</code> 值提供了一个轻量级代理，<code v-pre>Optional</code> 对象可以防止你的代码直接抛出 <code v-pre>NullPointException</code>。</p>
<p>虽然 <code v-pre>Optional</code> 是 Java 8 为了支持流式编程才引入的，但其实它是一个通用的工具。为了证明这点，在本节中，我们会把它用在普通的类中。因为涉及一些运行时检测，所以把这一小节放在了本章。</p>
<p>实际上，在所有地方都使用 <code v-pre>Optional</code> 是没有意义的，有时候检查一下是不是 <code v-pre>null</code> 也挺好的，或者有时我们可以合理地假设不会出现 <code v-pre>null</code>，甚至有时候检查 <code v-pre>NullPointException</code> 异常也是可以接受的。<code v-pre>Optional</code> 最有用武之地的是在那些“更接近数据”的地方，在问题空间中代表实体的对象上。举个简单的例子，很多系统中都有 <code v-pre>Person</code> 类型，代码中有些情况下你可能没有一个实际的 <code v-pre>Person</code> 对象（或者可能有，但是你还没用关于那个人的所有信息）。这时，在传统方法下，你会用到一个 <code v-pre>null</code> 引用，并且在使用的时候测试它是不是 <code v-pre>null</code>。而现在，我们可以使用 <code v-pre>Optional</code>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Person.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>&lt;Empty>
Smith
Bob Smith
Bob Smith 11 Degree Lane, Frostbite Falls, MN
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code v-pre>Person</code> 的设计有时候又叫“数据传输对象（DTO，data-transfer object）”。注意，所有字段都是 <code v-pre>public</code> 和 <code v-pre>final</code> 的，所以没有 <code v-pre>getter</code> 和 <code v-pre>setter</code> 方法。也就是说，<code v-pre>Person</code> 是不可变的，你只能通过构造器给它赋值，之后就只能读而不能修改它的值（字符串本身就是不可变的，因此你无法修改字符串的内容，也无法给它的字段重新赋值）。如果你想修改一个 <code v-pre>Person</code>，你只能用一个新的 <code v-pre>Person</code> 对象来替换它。<code v-pre>empty</code> 字段在对象创建的时候被赋值，用于快速判断这个 <code v-pre>Person</code> 对象是不是空对象。</p>
<p>如果想使用 <code v-pre>Person</code>，就必须使用 <code v-pre>Optional</code> 接口才能访问它的 <code v-pre>String</code> 字段，这样就不会意外触发 <code v-pre>NullPointException</code> 了。</p>
<p>现在假设你已经因你惊人的理念而获得了一大笔风险投资，现在你要招兵买马了，但是在虚位以待时，你可以将 <code v-pre>Person Optional</code> 对象放在每个 <code v-pre>Position</code> 上：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Position.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Position: CEO, Employee: &lt;Empty>
Position: Programmer, Employee: Arthur Fonzarelli
caught EmptyTitleException
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里使用 <code v-pre>Optional</code> 的方式不太一样。请注意，<code v-pre>title</code> 和 <code v-pre>person</code> 都是普通字段，不受 <code v-pre>Optional</code> 的保护。但是，修改这些字段的唯一途径是调用 <code v-pre>setTitle()</code> 和 <code v-pre>setPerson()</code> 方法，这两个都借助 <code v-pre>Optional</code> 对字段进行了严格的限制。</p>
<p>同时，我们想保证 <code v-pre>title</code> 字段永远不会变成 <code v-pre>null</code> 值。为此，我们可以自己在 <code v-pre>setTitle()</code> 方法里边检查参数 <code v-pre>newTitle</code> 的值。但其实还有更好的做法，函数式编程一大优势就是可以让我们重用经过验证的功能（即便是个很小的功能），以减少自己手动编写代码可能产生的一些小错误。所以在这里，我们用 <code v-pre>ofNullable()</code> 把 <code v-pre>newTitle</code> 转换一个 <code v-pre>Optional</code>（如果传入的值为 <code v-pre>null</code>，<code v-pre>ofNullable()</code> 返回的将是 <code v-pre>Optional.empty()</code>）。紧接着我们调用了 <code v-pre>orElseThrow()</code> 方法，所以如果 <code v-pre>newTitle</code> 的值是 <code v-pre>null</code>，你将会得到一个异常。这里我们并没有把 <code v-pre>title</code> 保存成 <code v-pre>Optional</code>，但通过应用 <code v-pre>Optional</code> 的功能，我们仍然如愿以偿地对这个字段施加了约束。</p>
<p><code v-pre>EmptyTitleException</code> 是一个 <code v-pre>RuntimeException</code>，因为它意味着程序存在错误。在这个方案里边，你仍然可能会得到一个异常。但不同的是，在错误产生的那一刻（向 <code v-pre>setTitle()</code> 传 <code v-pre>null</code> 值时）就会抛出异常，而不是发生在其它时刻，需要你通过调试才能发现问题所在。另外，使用 <code v-pre>EmptyTitleException</code> 还有助于定位 BUG。</p>
<p><code v-pre>Person</code> 字段的限制又不太一样：如果你把它的值设为 <code v-pre>null</code>，程序会自动把将它赋值成一个空的 <code v-pre>Person</code> 对象。先前我们也用过类似的方法把字段转换成 <code v-pre>Optional</code>，但这里我们是在返回结果的时候使用 <code v-pre>orElse(new Person())</code> 插入一个空的 <code v-pre>Person</code> 对象替代了 <code v-pre>null</code>。</p>
<p>在 <code v-pre>Position</code> 里边，我们没有创建一个表示“空”的标志位或者方法，因为 <code v-pre>person</code> 字段的 <code v-pre>Person</code> 对象为空，就表示这个 <code v-pre>Position</code> 是个空缺位置。之后，你可能会发现你必须添加一个显式的表示“空位”的方法，但是正如 YAGNI[^2] (You Aren't Going to Need It，你永远不需要它)所言，在初稿时“实现尽最大可能的简单”，直到程序在某些方面要求你为其添加一些额外的特性，而不是假设这是必要的。</p>
<p>请注意，虽然你清楚你使用了 <code v-pre>Optional</code>，可以免受 <code v-pre>NullPointerExceptions</code> 的困扰，但是 <code v-pre>Staff</code> 类却对此毫不知情。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Staff.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>[Position: President, Employee: Me Last The Top, Lonely
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>注意，在有些地方你可能还是要测试引用是不是 <code v-pre>Optional</code>，这跟检查是否为 <code v-pre>null</code> 没什么不同。但是在其它地方（例如本例中的 <code v-pre>toString()</code> 转换），你就不必执行额外的测试了，而可以直接假设所有对象都是有效的。</p>
<h3 id="标记接口" tabindex="-1"><a class="header-anchor" href="#标记接口" aria-hidden="true">#</a> 标记接口</h3>
<p>有时候使用一个<strong>标记接口</strong>来表示空值会更方便。标记接口里边什么都没有，你只要把它的名字当做标签来用就可以。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// onjava/Null.java</span>
<span class="token keyword">package</span> <span class="token namespace">onjava</span><span class="token punctuation">;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果你用接口取代具体类，那么就可以使用 <code v-pre>DynamicProxy</code> 来自动地创建 <code v-pre>Null</code> 对象。假设我们有一个 <code v-pre>Robot</code> 接口，它定义了一个名字、一个模型和一个描述 <code v-pre>Robot</code> 行为能力的 <code v-pre>List&lt;Operation&gt;</code>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Robot.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">interface</span> <span class="token class-name">Robot</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>你可以通过调用 <code v-pre>operations()</code> 来访问 <code v-pre>Robot</code> 的服务。<code v-pre>Robot</code> 里边还有一个 <code v-pre>static</code> 方法来执行测试。</p>
<p><code v-pre>Operation</code> 包含一个描述和一个命令（这用到了<strong>命令模式</strong>）。它们被定义成函数式接口的引用，所以可以把 lambda 表达式或者方法的引用传给 <code v-pre>Operation</code> 的构造器：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/Operation.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">Operation</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>现在我们可以创建一个扫雪 <code v-pre>Robot</code>：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/SnowRemovalRobot.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Robot name: Slusher
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>假设存在许多不同类型的 <code v-pre>Robot</code>，我们想让每种 <code v-pre>Robot</code> 都创建一个 <code v-pre>Null</code> 对象来执行一些特殊的操作——在本例中，即提供 <code v-pre>Null</code> 对象所代表 <code v-pre>Robot</code> 的确切类型信息。这些信息是通过动态代理捕获的：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/NullRobot.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>Robot name: SnowBee
Robot model: SnowBot Series 11
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>无论何时，如果你需要一个空 <code v-pre>Robot</code> 对象，只需要调用 <code v-pre>newNullRobot()</code>，并传递需要代理的 <code v-pre>Robot</code> 的类型。这个代理满足了 <code v-pre>Robot</code> 和 <code v-pre>Null</code> 接口的需要，并提供了它所代理的类型的确切名字。</p>
<h3 id="mock-对象和桩" tabindex="-1"><a class="header-anchor" href="#mock-对象和桩" aria-hidden="true">#</a> Mock 对象和桩</h3>
<p><strong>Mock 对象</strong>和 **桩（Stub）**在逻辑上都是 <code v-pre>Optional</code> 的变体。他们都是最终程序中所使用的“实际”对象的代理。不过，Mock 对象和桩都是假扮成那些可以传递实际信息的实际对象，而不是像 <code v-pre>Optional</code> 那样把包含潜在 <code v-pre>null</code> 值的对象隐藏。</p>
...
<!-- Interfaces and Type -->
<h2 id="接口和类型" tabindex="-1"><a class="header-anchor" href="#接口和类型" aria-hidden="true">#</a> 接口和类型</h2>
<p><code v-pre>interface</code> 关键字的一个重要目标就是允许程序员隔离组件，进而降低耦合度。使用接口可以实现这一目标，但是通过类型信息，这种耦合性还是会传播出去——接口并不是对解耦的一种无懈可击的保障。比如我们先写一个接口：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/interfacea/A.java</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后实现这个接口，你可以看到其代码是怎么从实际类型开始顺藤摸瓜的：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/InterfaceViolation.java</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>通过使用 RTTI，我们发现 <code v-pre>a</code> 是用 <code v-pre>B</code> 实现的。通过将其转型为 <code v-pre>B</code>，我们可以调用不在 <code v-pre>A</code> 中的方法。</p>
<p>这样的操作完全是合情合理的，但是你也许并不想让客户端开发者这么做，因为这给了他们一个机会，使得他们的代码与你的代码的耦合度超过了你的预期。也就是说，你可能认为 <code v-pre>interface</code> 关键字正在保护你，但其实并没有。另外，在本例中使用 <code v-pre>B</code> 来实现 <code v-pre>A</code> 这种情况是有公开案例可查的[^3]。</p>
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/packageaccess/HiddenC.java</span>
...
<span class="token keyword">public</span> <span class="token keyword">class</span> <span class="token class-name">HiddenC</span> <span class="token punctuation">{</span>
    ...
    <span class="token punctuation">}</span>
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个包中唯一 <code v-pre>public</code> 的部分就是 <code v-pre>HiddenC</code>，在被调用时将产生 <code v-pre>A</code>接口类型的对象。这里有趣之处在于：即使你从 <code v-pre>makeA()</code> 返回的是 <code v-pre>C</code> 类型，你在包的外部仍旧不能使用 <code v-pre>A</code> 之外的任何方法，因为你不能在包的外部命名 <code v-pre>C</code>。</p>
<p>现在如果你试着将其向下转型为 <code v-pre>C</code>，则将被禁止，因为在包的外部没有任何 <code v-pre>C</code> 类型可用：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/HiddenImplementation.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>public C.f()
typeinfo.packageaccess.C
public C.g()
package C.u()
protected C.v()
private C.w()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>正如你所看到的，通过使用反射，仍然可以调用所有方法，甚至是 <code v-pre>private</code> 方法！如果知道方法名，你就可以在其 <code v-pre>Method</code> 对象上调用 <code v-pre>setAccessible(true)</code>，就像在 <code v-pre>callHiddenMethod()</code> 中看到的那样。</p>
<p>你可能觉得，可以通过只发布编译后的代码来阻止这种情况，但其实这并不能解决问题。因为只需要运行 <code v-pre>javap</code>（一个随 JDK 发布的反编译器）即可突破这一限制。下面是一个使用 <code v-pre>javap</code> 的命令行：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>javap <span class="token parameter variable">-private</span> C
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p><code v-pre>-private</code> 标志表示所有的成员都应该显示，甚至包括私有成员。下面是输出：</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>class typeinfo.packageaccess.C extends
...
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/InnerImplementation.java</span>
...
protected C.v()
private C.w()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里对反射仍然没有任何东西可以隐藏。那么如果是匿名类呢？</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/AnonymousImplementation.java</span>
...
protected C.v()
private C.w()
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>看起来任何方式都没法阻止反射调用那些非公共访问权限的方法。对于字段来说也是这样，即便是 <code v-pre>private</code> 字段：</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token comment">// typeinfo/ModifyingPrivateFields.java</span>
...
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>i = 1, I'm totally safe, Am I safe?
f.getInt(pf): 1
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>但实际上 <code v-pre>final</code> 字段在被修改时是安全的。运行时系统会在不抛出异常的情况下接受任何修改的尝试，但是实际上不会发生任何修改。</p>
<p>通常，所有这些违反访问权限的操作并不是什么十恶不赦的。如果有人使用这样的技术去调用标志为 <code v-pre>private</code> 或包访问权限的方法（很明显这些访问权限表示这些人不应该调用它们），那么对他们来说，如果你修改了这些方法的某些地方，他们不应该抱怨。另一方面，总是在类中留下后门，也许会帮助你解决某些特定类型的问题（这些问题往往除此之外，别无它法）。总之，不可否认，反射给我们带来了很多好处。</p>
...
<p>RTTI 允许通过匿名类的引用来获取类型信息。初学者极易误用它，因为在学会使用多态调用方法之前，这么做也很有效。有过程化编程背景的人很容易把程序组织成一系列 <code v-pre>switch</code> 语句，你可以用 RTTI 和 <code v-pre>switch</code> 实现功能，但这样就损失了多态机制在代码开发和维护过程中的重要价值。面向对象编程语言是想让我们尽可能地使用多态机制，只在非用不可的时候才使用 RTTI。</p>
<p>然而使用多态机制的方法调用，要求我们拥有基类定义的控制权。因为在你扩展程序的时候，可能会发现基类并未包含我们想要的方法。如果基类来自别人的库，这时 RTTI 便是一种解决之道：可继承一个新类，然后添加你需要的方法。在代码的其它地方，可以检查你自己特定的类型，并调用你自己的方法。这样做不会破坏多态性以及程序的扩展能力，因为这样添加一个新的类并不需要修改程序中的 <code v-pre>switch</code> 语句。但如果想在程序中增加具有新特性的代码，你就必须使用 RTTI 来检查这个特定的类型。</p>
<p>如果只是为了方便某个特定的类，就将某个特性放进基类里边，这将使得从那个基类派生出的所有其它子类都带有这些可能毫无意义的东西。这会导致接口更加不清晰，因为我们必须覆盖从基类继承而来的所有抽象方法，事情就变得很麻烦。举个例子，现在有一个表示乐器 <code v-pre>Instrument</code> 的类层次结构。假设我们想清理管弦乐队中某些乐器残留的口水，一种办法是在基类 <code v-pre>Instrument</code> 中放入 <code v-pre>clearSpitValve()</code> 方法。但这样做会导致类结构混乱，因为这意味着打击乐器 <code v-pre>Percussion</code>、弦乐器 <code v-pre>Stringed</code> 和电子乐器 <code v-pre>Electronic</code> 也需要清理口水。在这个例子中，RTTI 可以提供一种更合理的解决方案。可以将 <code v-pre>clearSpitValve()</code> 放在某个合适的类中，在这个例子中是管乐器 <code v-pre>Wind</code>。不过，在这里你可能会发现还有更好的解决方法，就是将 <code v-pre>prepareInstrument()</code> 放在基类中，但是初次面对这个问题的读者可能想不到还有这样的解决方案，而误认为必须使用 RTTI。</p>
...
<p>我们已经看到，反射，因其更加动态的编程风格，为我们开创了编程的新世界。但对有些人来说，反射的动态特性却是一种困扰。对那些已经习惯于静态类型检查的安全性的人来说，Java 中允许这种动态类型检查（只在运行时才能检查到，并以异常的形式上报检查结果）的操作似乎是一种错误的方向。有些人想得更远，他们认为引入运行时异常本身就是一种指示，指示我们应该避免这种代码。我发现这种意义的安全是一种错觉，因为总是有些事情是在运行时才发生并抛出异常的，即使是在那些不包含任何 <code v-pre>try</code> 语句块或异常声明的程序中也是如此。因此，我认为一致性错误报告模型的存在使我们能够通过使用反射编写动态代码。当然，尽力编写能够进行静态检查的代码是有价值的，只要你有这样的能力。但是我相信动态代码是将 Java 与其它诸如 C++ 这样的语言区分开的重要工具之一。</p>
...
<p>[^4]: 比如，Python 中在元素前面添加双下划线 <code v-pre>__</code>，就表示你想隐藏这个元素。如果你在类或者包外面调用了这个元素，运行环境就会报错。</p>
