...
<p>这种情况下，我们可以借助第三方工具[^9]，但使用 Scala 或 Clojure 这样的语言可能更简单。因为它们从一开始就是为保持不变性而设计的。你可以采用这些语言来编写你的 Java 项目的一部分。如果必须要用纯函数式编写，则可以用 Scala（需要一些练习） 或 Clojure （仅需更少的练习）。虽然 Java 支持<RouterLink to="/24-Concurrent-Programming.html">并发编程</RouterLink>，但如果这是你项目的核心部分，你应该考虑在项目部分功能中使用 <code v-pre>Scala</code> 或 <code v-pre>Clojure</code> 之类的语言。</p>
<!-- Summary -->
...
[^4]: 这个语法来自 C++。
[^5]: 我还没有验证过这种说法。
[^6]: 当你理解了<RouterLink to="/24-Concurrent-Programming.html">并发编程</RouterLink>章节的内容，你就能明白为什么更改共享变量 “不是线程安全的” 的了。
