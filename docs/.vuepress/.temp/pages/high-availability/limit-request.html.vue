<template><div><h1 id="限流" tabindex="-1"><a class="header-anchor" href="#限流" aria-hidden="true">#</a> 限流</h1>
<h2 id="何为限流-为什么要限流" tabindex="-1"><a class="header-anchor" href="#何为限流-为什么要限流" aria-hidden="true">#</a> 何为限流？为什么要限流？</h2>
...
<p>例如我们的借口限流每分钟处理 60 个请求，我们可以把 1 分钟分为 60 个窗口。每隔 1 秒移动一次，每个窗口一秒只能处理 不大于 <code v-pre>60(请求数)/60（窗口数）</code> 的请求， 如果当前窗口的请求计数总和超过了限制的数量的话就不再处理其他请求。</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>get <span class="token number">1</span> tokens: <span class="token number">0</span>.0s
